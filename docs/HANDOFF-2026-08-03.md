...
Pinned: **`worktree-pre-push-scoped-pytest` @ `43e4d530`**.

| Fact | Value | Command |
|---|---|---|
| RC HEAD | `43e4d530` | `git rev-parse HEAD` |
| vs `origin/main` (`dd692561`) | **23 ahead, 0 behind** | `git rev-list --count origin/main..HEAD` |
| vs RC remote (`3808f63f`) | **13 ahead, 0 behind** | `git rev-list --count origin/worktree-...` |
...
against this exact SHA. NO CI has ever run on `43e4d530` or on any commit in this
...
### Local receipts on the exact RC HEAD `43e4d530`
...
### Every held commit (23, unpushed to main)
...
PRECONDITION ON GATE 3: **PARTIALLY MET.** Small is verified by a receipt
any reader can reproduce; high is verified by a PRIVATE hash-bound receipt
(details below); medium is not verified at all.
...
| medium `multifail-1-two-modules` | -- | -- | **NOT ATTEMPTED**, and no private artifacts placed |
...
| `multifail-1-two-modules` | -- | -- | -- | NOT AVAILABLE, no artifacts placed |

So the precondition is **PARTIALLY met**: high is verified and replayable,
medium is not. Spending against the high tier is now defensible on
evidence; spending against medium is not, and the two should not be
