...
PRECONDITION ON GATE 3: **NOT MET.** Only the SMALL tier is verified by a
...
| medium `multifail-1-two-modules` | -- | -- | **NOT ATTEMPTED** |
| high `hard-1-order-api` | -- | -- | **NOT ATTEMPTED** |
...
WHAT WOULD ACTUALLY MEET THIS PRECONDITION: a non-contaminating,
hash-bound, reproducible private receipt for the medium and high tiers --
artifacts stored outside this repository (so the held-out design is not
contaminated), content-hashed, with the hash and both exit codes recorded
here so any later run can be checked against it. Until that exists, treat
medium and high as unverified and do not spend against them.
