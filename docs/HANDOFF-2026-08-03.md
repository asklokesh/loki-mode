...
Pinned: **`worktree-pre-push-scoped-pytest` @ `7b25a772`**.

| Fact | Value | Command |
|---|---|---|
| RC HEAD | `7b25a772` | `git rev-parse HEAD` |
| vs `origin/main` (`dd692561`) | **16 ahead, 0 behind** | `git rev-list --count origin/main..HEAD` |
| vs RC remote (`3808f63f`) | **6 ahead, 0 behind** | `git rev-list --count origin/worktree-...` |

A steering note pinned `ea930bbb` at 15/5. That was exact when written; one
commit landed after it, so the live numbers are above. The RC remote branch
`3808f63f` is 6 commits stale -- it was pushed before the interlock work.
...
Getting a CI verdict on this exact SHA requires a branch push **and** a
PR to main. Both are founder-gated and are listed as ONE gate below,
because either alone does not produce the evidence.
...
| Check | Result |
|---|---|
| `python3 -m pytest tests/ -q -k "bench or schema"` | 149 passed |
...
### Every held commit (16, unpushed to main)
