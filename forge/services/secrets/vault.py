...
def rotate_value(forge_dir: str, name: str,
    ...
    N-84: when `rotated_by_user_id` is supplied, the rotations.jsonl
    line records the actor for audit.
    """
    if not _KEY_RE_OK(name):
    ...
    try:
        ...
        rec = {"name": name, "ts": int(time.time()),
               "kind": "value_rotated"}
        if rotated_by_user_id:
            rec["rotated_by_user_id"] = rotated_by_user_id
...
def list_rotations(forge_dir: str, *,
    ...
    history.
    N-135: optional `since_ts=` filter returns only rotations newer
    than the unix timestamp (handy for "last 7 days" dashboards)."""
