...
`),!1}function E$($){if(!Number.isFinite($)||$<=0)return!1;try{return process.kill($,0),!0}catch{return!1}}function N$($){if(!y($))return null;try{let Q=W$($,"utf-8").trim();if(!Q)return null;let Z=Number.parseInt(Q,10);return Number.isFinite(Z)?Z:null}catch{return null}}function vQ($){let Q=[],Z=N$(D($,"loki.pid"));if(Z!==null&&E$(Z))Q.push(`global:${Z}`);let z=D($,"sessions");if(y(z)){let X=[];try{X=d1(z)}catch{X=[]}for(let q of X){let K=D(z,q);try{if(!o1(K).isDirectory())continue}catch{continue}let W=D(K,"loki.pid"),J=N$(W);if(J!==null&&E$(J))Q.push(`${q}:${J}`)}}if(y($)){let X=[];try{X=d1($)}catch{X=[]}for(let q of X){if(!q.startsWith("run-")||!q.endsWith(".pid"))continue;let K=D($,q);try{if(!o1(K).isFile())continue}catch{continue}let W=CQ(q,".pid").slice(4),J=N$(K);if(J!==null&&E$(J)){if(!Q.some((H)=>H.startsWith(`${W}:`)))Q.push(`${W}:${J}`)}}}return Q}async function s1($,Q){let Z=await R(["jq","-r",$,Q]);if(Z.exitCode!==0)return null;return Z.stdout.trim()}function r1($,Q){try{let Z=W$($,"utf-8"),X=JSON.parse(Z)[Q];if(typeof X==="number"){if(Q==="budget_used"){let q=Math.round(X*100)/100;if(Number.isInteger(q))return String(q);return String(q)}return String(X)}if(X===void 0||X===null)return"0";return String(X)}catch{return"0"}}function t1($,Q,Z){try{let z=W$($,"utf-8"),q=JSON.parse(z)[Q];if(typeof q==="number"&&Number.isFinite(q))return q;return Z}catch{return Z}}async function mQ(){let $=j();if(!await yQ())return 1;if(!y($))return process.stdout.write(`${k}Loki Mode Status${V}
...
`),1;let Q=g,Z=j(),z=process.env.LOKI_DASHBOARD_PORT||"57374",X=process.env.LOKI_PROVIDER||"claude",q=await R([$,"-c",hQ,Q,Z,z,X],{timeoutMs:30000});if(q.exitCode!==0)return process.stderr.write(`{"error": "Failed to generate JSON status. Ensure python3 is available."}
...
Start a session with: loki start <prd>`}}let X=nQ(z);return{exitCode:0,stdout:Q?aQ(X,Z):sQ(X,Z)}}async function rQ($){let{emitDeprecatedAlias:Q}=await Promise.resolve().then(() => (Q1(),Z0));Q("stats","report session",$);let Z=K0($);return console.log(Z.stdout),Z.exitCode}var V0=P(()=>{C();c()});var T0={};b(T0,{runDoctor:()=>U3,pythonImportOk:()=>J1,httpReachable:()=>q1,checkTool:()=>B0,checkSkills:()=>G0,checkDisk:()=>V1,buildDoctorJson:()=>M0,_setPythonImportOkForTest:()=>z3});import{existsSync as J0,lstatSync as tQ,readlinkSync as iQ,statfsSync as eQ}from"fs";import{spawnSync as $3}from"child_process";import{homedir as W0}from"os";import{resolve as K1}from"path";function Z3($){let Q=$.match(Q3);return Q?Q[1]:null}async function U0($){try{let Q=await R([$,"--version"],{timeoutMs:5000}),Z=(Q.stdout||Q.stderr||"").trim();return Z3(Z)}catch{return null}}function H0($,Q){let Z=$.split(".").map((X)=>parseInt(X,10)),z=Q.split(".").map((X)=>parseInt(X,10));while(Z.length<2)Z.push(0);while(z.length<2)z.push(0);for(let X=0;X<2;X++){let q=Z[X]??0,K=z[X]??0;if(Number.isNaN(q)||Number.isNaN(K))return 0;if(q!==K)return q-K}return 0}async function B0($,Q,Z,z=null){let X=await f(Q),q=X!==null,K=q?await U0(Q):null,W="pass";if(!q)W=Z==="required"?"fail":"warn";else if(z&&K){if(H0(K,z)<0)W=Z==="required"?"fail":"warn"}return{name:$,command:Q,found:q,version:K,required:Z,min_version:z,status:W,path:X}}function V1(){let $=null;try{let Z=eQ(W0()),z=Number(Z.bavail)*Number(Z.bsize);$=Math.round(z/1073741824*10)/10}catch{$=null}let Q="pass";if($!==null){if($<1)Q="fail";else if($<5)Q="warn"}return{available_gb:$,status:Q}}async function q1($,Q=2000){try{return(await fetch($,{signal:AbortSignal.timeout(Q)})).ok}catch{return!1}}async function J1($,Q=!1){let Z=`import ${$}`,z=Q?30000:5000;if(!Q)return(await z$(Z,{timeoutMs:z})).exitCode===0;let X=await Z$();if(!X)return!1;return(await R([X,"-c",Z],{timeoutMs:z})).exitCode===0}function z3($){C$.fn=$??J1}function G0(){let $=W0();return X3.map(({name:Q,dir:Z})=>{let z=K1($,Z),X=z,q=K1(z,"SKILL.md");if(J0(q))return{name:Q,path:X,status:"pass",detail:""};try{if(tQ(z).isSymbolicLink()){let W="unknown";try{W=iQ(z)}catch{}return{name:Q,path:X,status:"fail",detail:`(broken symlink -> ${W})`}}}catch{}return{name:Q,path:X,status:"warn",detail:"(not found - run 'loki setup-skill')"}})}async function Y0(){return Promise.all(K3.map(async($)=>{return{...await B0($.jsonName,$.cmd,$.required,$.min??null),displayName:$.displayName}}))}async function q3(){let Q=await f("sentrux")!==null,Z=Q?await U0("sentrux"):null;return{found:Q,version:Z,status:Q?"pass":"warn",required:"optional"}}async function V3(){let{openSync:$,statSync:Q,readSync:Z,closeSync:z,existsSync:X}=await import("fs"),{join:q}=await import("path"),K=65536,W=process.env.LOKI_DIR??".loki",J=q(W,"memory",".errors.log"),U=[],H=!1;try{if(X(J)){H=!0;let Y=Q(J).size,B=Math.max(0,Y-65536),G=Y-B,O=Buffer.alloc(G),M=$(J,"r");try{Z(M,O,0,G,B)}finally{z(M)}let x=O.toString("utf-8").split(`
`);if(B>0&&x.length>0)x=x.slice(1);x=x.map((v)=>v.trim()).filter((v)=>v.length>0),U=x.slice(-5)}}catch{U=[]}return{errors_log_path:H?J:null,recent_errors:U,recent_error_count:U.length,status:U.length===0?"pass":"warn"}}async function M0(){let Q=(await Y0()).map(({displayName:J,...U})=>U),Z=V1(),z=await q3(),X=await V3(),q=0,K=0,W=0;for(let J of Q)if(J.status==="pass")q++;else if(J.status==="fail")K++;else W++;if(Z.status==="pass")q++;else if(Z.status==="fail")K++;else W++;return{loki_mode_version:R$(),checks:Q,disk:Z,sentrux:z,memory:X,summary:{passed:q,failed:K,warnings:W,ok:K===0}}}function A($){switch($){case"pass":return`${S}PASS${V}`;case"fail":return`${T}FAIL${V}`;case"warn":return`${_}WARN${V}`}}function S$($){let Q=$.version?` (v${$.version})`:"",Z=$.displayName;if(!$.found){let z=$.required==="required"?"not found":$.required==="recommended"?"not found (recommended)":"not found (optional)";return`  ${A($.status)}  ${Z} - ${z}`}if($.min_version&&$.version&&H0($.version,$.min_version)<0){let z=$.required==="required"?"requires":"recommended";return`  ${A($.status)}  ${Z}${Q} - ${z} >= ${$.min_version}`}return`  ${A($.status)}  ${Z}${Q}`}function D$($,Q){if(Q==="pass")$.pass++;else if(Q==="fail")$.fail++;else $.warn++}function J3(){process.stdout.write(`${k}loki doctor${V} - Check system prerequisites
...
`)}async function W3(){process.stdout.write(`${k}Loki Mode Doctor${V}
...
`),$.fail++,process.stdout.isTTY){let w=K1(g,"autonomy/provider-offer.sh");if(J0(w))$3("bash",[w,"report"],{stdio:"inherit"})}}process.stdout.write(`
...
`),Q}catch(Z){if(Q!==null){try{B1(Q)}catch{}try{p0($)}catch{}}if(Z?.code==="EEXIST")return null;throw Z}}function H8($,Q){let Z;try{Z=$8($)}catch{return!0}if(Z.isSymbolicLink())try{return p0($),!0}catch{return!1}let z;try{z=u0($,"r")}catch{return!0}try{let X=e3(z);if(Date.now()-X.mtimeMs<Q)return!1;let K=NaN;try{let W=Buffer.alloc(64),J=Q8(z,W,0,64,0);K=Number.parseInt(W.subarray(0,J).toString("utf-8").trim(),10)}catch{}if(Number.isFinite(K)&&W8(K))return!1;try{if(z8($).mtimeMs>X.mtimeMs)return!1}catch{return!0}try{c0($,{force:!0})}catch{}return!0}finally{try{B1(z)}catch{}}}function w$($,Q,Z={}){let z=Z.timeoutMs??1e4,X=Z.pollMs??25,q=Z.staleMs??30000,K=J8($),W=Date.now()+z,J=null,U=0,H=new Int32Array(new SharedArrayBuffer(4));while(J===null){if(J=U8(K),J!==null)break;if(Date.now()>W)throw Error(`withFileLockSync: timed out after ${z}ms acquiring ${K}`);if(H8(K,q))continue;let Y=Math.min(X*2**Math.min(U,4),V8);U+=1,Atomics.wait(H,0,0,Y)}try{return Q()}finally{try{B1(J)}catch{}try{c0(K,{force:!0})}catch{}}}var q8=0,v$,V8=50;var m$=P(()=>{v$=new Map});import{existsSync as K$,mkdirSync as G$,copyFileSync as s0,readFileSync as M1,readdirSync as B8,statSync as G8,writeFileSync as Y8,renameSync as r0,appendFileSync as t0,rmSync as M8}from"fs";import{join as m,dirname as g$}from"path";function O8($){let Q=n0.then($,$);return n0=Q.catch((Z)=>{console.warn("[checkpoint] serialized op rejected:",Z);return}),Q}function o($){return m($,"state","checkpoints")}function i0($){return m(o($),"index.jsonl")}async function A8($){let Q=await R(["git","rev-parse","HEAD"],{cwd:$,timeoutMs:5000});if(Q.exitCode!==0)return"no-git";return Q.stdout.trim()||"no-git"}async function w8($){let Q=await R(["git","branch","--show-current"],{cwd:$,timeoutMs:5000});if(Q.exitCode!==0)return"unknown";return Q.stdout.trim()||"unknown"}async function _8($){let Q=await R(["git","diff","--quiet"],{cwd:$,timeoutMs:5000}),Z=await R(["git","diff","--cached","--quiet"],{cwd:$,timeoutMs:5000}),z=Q.exitCode===1,X=Z.exitCode===1;return z||X}function I8($){let Q=m($,"state","orchestrator.json");if(!K$(Q))return"unknown";try{let z=JSON.parse(M1(Q,"utf-8")).currentPhase;return typeof z==="string"&&z.length>0?z:"unknown"}catch{return"unknown"}}function P8($,Q){for(let Z of L8){let z=m($,Z);if(!K$(z))continue;let X=m(Q,Z);G$(g$(X),{recursive:!0});try{s0(z,X)}catch{}}}function $6($,Q){G$(g$($),{recursive:!0});let Z=`${$}.tmp.${process.pid}.${++e0}`;Y8(Z,Q),r0(Z,$)}function j8($){return JSON.stringify($,null,2)}function Q6($){return`{${[`"id": ${JSON.stringify($.id)}`,`"ts": ${JSON.stringify($.ts)}`,`"iter": ${JSON.stringify($.iter)}`,`"task": ${JSON.stringify($.task)}`,`"sha": ${JSON.stringify($.sha)}`].join(", ")}}`}async function F8($){return O8(()=>R8($))}async function R8($){let Q=$.lokiDirOverride??j(),Z=process.cwd(),z=o(Q);if(G$(z,{recursive:!0}),!$.forceCreate){if(!await _8(Z))return{created:!1,reason:"no uncommitted changes"}}let X=await A8(Z),q=await w8(Z),K=$.iteration??Number.parseInt(process.env.ITERATION_COUNT??"0",10),W=$.epochOverride??Math.floor(Date.now()/1000),J=`cp-${K}-${W}`,U=m(z,J);G$(U,{recursive:!0}),P8(Q,U);let H=new Date().toISOString().replace(/\.\d{3}Z$/,"Z"),Y=($.taskDescription??"task completed").slice(0,T8),B=$.provider??process.env.PROVIDER_NAME??"claude",G={id:J,timestamp:H,iteration:K,task_id:$.taskId??"unknown",task_description:Y,git_sha:X,git_branch:q,provider:B,phase:I8(Q)};$6(m(U,"metadata.json"),j8(G));let O={id:G.id,ts:G.timestamp,iter:G.iteration,task:G.task_description,sha:G.git_sha},M=i0(Q);return w$(M,()=>{t0(M,`${Q6(O)}
`)}),k8(Q),{created:!0,id:J,metadata:G,dir:U}}function T1($){let Q=o($);if(!K$(Q))return[];return B8(Q).filter((Z)=>Z.startsWith("cp-")).filter((Z)=>{try{return G8(m(Q,Z)).isDirectory()}catch{return!1}})}function O1($){return[...$].sort((Q,Z)=>{let z=a0(Q),X=a0(Z);return z-X})}function a0($){let Q=$.split("-");if(Q.length<3)return 0;let Z=Q[Q.length-1],z=Number.parseInt(Z??"0",10);return Number.isFinite(z)?z:0}function k8($){let Q=T1($);if(Q.length<=o0)return;let Z=O1(Q),z=Z.slice(0,Z.length-o0);for(let X of z)try{M8(m(o($),X),{recursive:!0,force:!0})}catch{}x8($)}function x8($){let Q=O1(T1($)),Z=[];for(let q of Q){let K=m(o($),q,"metadata.json"),W=m(o($),q);if(!K$(K)){G1($,W,"missing_field","metadata.json");continue}try{let J=JSON.parse(M1(K,"utf-8")),U=z6(J,K);if(!U.ok){G1($,W,U.reason,U.field);continue}let H=U.value;Z.push(Q6({id:H.id,ts:H.timestamp,iter:H.iteration,task:H.task_description??"",sha:H.git_sha}))}catch{G1($,W,"invalid_type","metadata.json")}}let z=i0($),X=Z.length>0?`${Z.join(`
`)}
`:"";$6(z,X)}function G1($,Q,Z,z){let X=m($,"events.jsonl"),q={timestamp:new Date().toISOString(),type:"checkpoint.metadata.dropped",checkpoint_dir:Q,reason:Z,field:z};try{G$(g$(X),{recursive:!0}),w$(X,()=>{t0(X,`${JSON.stringify(q)}
`)})}catch{}}function A1($){let Q=$??j(),Z=O1(T1(Q)),z=[];for(let X of Z){let q=Z6(Q,X);if(q)z.push(q)}return z}function Z6($,Q){let Z=m(o($),Q,"metadata.json");if(!K$(Z))return null;try{let z=JSON.parse(M1(Z,"utf-8"));return E8(z,Z)}catch{return null}}function E8($,Q){let Z=z6($,Q);return Z.ok?Z.value:null}function z6($,Q){if($===null||typeof $!=="object")return console.warn(`[checkpoint] invalid metadata at ${Q}: not an object`),{ok:!1,reason:"invalid_type",field:"<root>"};let Z=$,z=["id","timestamp","task_id","task_description","git_sha","git_branch","provider","phase"];for(let X of z){if(!(X in Z))return console.warn(`[checkpoint] invalid metadata at ${Q}: field "${X}" missing`),{ok:!1,reason:"missing_field",field:X};if(typeof Z[X]!=="string")return console.warn(`[checkpoint] invalid metadata at ${Q}: field "${X}" not a string`),{ok:!1,reason:"invalid_type",field:X}}if(!Object.prototype.hasOwnProperty.call(Z,"iteration"))return console.warn(`[checkpoint] invalid metadata at ${Q}: field "iteration" missing`),{ok:!1,reason:"missing_field",field:"iteration"};if(typeof Z.iteration!=="number"||!Number.isFinite(Z.iteration))return console.warn(`[checkpoint] invalid metadata at ${Q}: field "iteration" not a finite number`),{ok:!1,reason:"invalid_type",field:"iteration"};for(let X of S8){let q=Z[X];if(N8.test(q))return console.warn(`[checkpoint] invalid metadata at ${Q}: field "${X}" contains control characters`),{ok:!1,reason:"control_chars",field:X}}return{ok:!0,value:{id:Z.id,timestamp:Z.timestamp,iteration:Z.iteration,task_id:Z.task_id,task_description:Z.task_description,git_sha:Z.git_sha,git_branch:Z.git_branch,provider:Z.provider,phase:Z.phase}}}function w1($,Q){if(!D8.test($))throw new X6($);let Z=Q??j(),z=m(o(Z),$);if(!K$(z))throw new Y1($);let X=Z6(Z,$);if(!X)throw new Y1($);return X}function K6($,Q){let Z=w1($,Q),z=Q??j(),X=m(o(z),$),q=[];for(let K of C8){let W=m(X,K);if(!K$(W))continue;q.push({from:W,to:m(z,K)})}return{id:$,metadata:Z,restore:q}}function b8($){let Q=[],Z=0;for(let z of $.restore)try{G$(g$(z.to),{recursive:!0});let X=`${z.to}.tmp.${process.pid}.${++e0}`;s0(z.from,X),r0(X,z.to),Z+=1}catch(X){Q.push(`${z.from} -> ${z.to}: ${X.message}`)}return{restored:Z,errors:Q}}async function q6($,Q,Z=!1){let z=null;try{let q=await F8({taskDescription:`pre-rollback snapshot (before restoring ${$.id})`,taskId:"rollback",forceCreate:!0,lokiDirOverride:Q});if(q.created)z=q.id}catch(q){let K=q instanceof Error?q.message:String(q);if(!Z)throw Error("pre-rollback snapshot failed ("+K+"); aborting rollback to preserve current state. Re-run with force to roll back anyway without a safety snapshot.");console.warn("[checkpoint] pre-rollback snapshot failed; proceeding due to force:",K)}let X=b8($);return{preRollbackSnapshotId:z,restored:X.restored,errors:X.errors}}var o0=50,T8=200,n0,L8,e0=0,N8,S8,D8,Y1,X6,C8;var V6=P(()=>{C();d();m$();n0=Promise.resolve();L8=["state/orchestrator.json","autonomy-state.json","queue/pending.json","queue/completed.json","queue/in-progress.json","queue/current-task.json","CONTINUITY.md"];N8=/[\x00-\x08\x0a-\x1f\x7f-\x9f]/,S8=["id","task_id","git_sha","git_branch","provider","phase"];D8=/^[a-zA-Z0-9_-]+$/;Y1=class Y1 extends Error{id;constructor($){super(`Checkpoint not found: ${$}`);this.id=$;this.name="CheckpointNotFoundError"}};X6=class X6 extends Error{id;constructor($){super(`Invalid checkpoint ID: must be alphanumeric, hyphens, underscores only (got: ${$})`);this.id=$;this.name="InvalidCheckpointIdError"}};C8=["state/orchestrator.json","queue/pending.json","queue/completed.json","queue/in-progress.json","queue/current-task.json","CONTINUITY.md"]});var U6={};b(U6,{runRollback:()=>h8});async function h8($){let Q=$[0],Z=$.slice(1);if(Q===void 0||Q==="help"||Q==="--help"||Q==="-h")return process.stdout.write(J6),Q===void 0?1:0;switch(Q){case"list":{let z=[...A1()].reverse();if(z.length===0)return process.stdout.write(`${_}No checkpoints found.${V}
...
`;var H6=P(()=>{V6();c()});function y8(){return process.env.LOKI_TIER||"oss"}function B6($){let Q=y8();if(Q==="oss")return{allowed:!0,notes:[]};if(!process.env.LOKI_LICENSE_KEY)return{allowed:!1,notes:[`${_}LOKI_TIER='${Q}' requested but no LOKI_LICENSE_KEY set.${V}`,`Hosted/enterprise license verification is not available yet (capability: ${$}).`,"OSS users: leave LOKI_TIER unset (or 'oss') -- everything stays free."]};return{allowed:!0,notes:[`${_}LOKI_LICENSE_KEY set but the verification backend is not available yet (R9 seam).${V}`]}}var G6=P(()=>{c()});var M6={};b(M6,{runProof:()=>t8});import{existsSync as _$,readdirSync as v8,readFileSync as Y6,mkdtempSync as m8,copyFileSync as g8,rmSync as f8}from"fs";import{join as e}from"path";import{tmpdir as u8}from"os";import{createInterface as c8}from"readline";import{readFile as p8}from"fs/promises";function $$($){return $&&typeof $==="object"?$:{}}function p($){return $===void 0||$===null?"-":String($)}function I$(){return e(j(),"proofs")}function _1($){let Q=e(I$(),$,"proof.json");if(!_$(Q))return null;try{return JSON.parse(Y6(Q,"utf8"))}catch{return{}}}function i($,Q){return $.length>=Q?$:$+" ".repeat(Q-$.length)}function d8(){let $=I$();if(!_$($))return process.stdout.write(`${_}No proofs found.${V} Run 'loki start' to generate one.
...
`),0}function a8($){return new Promise((Q)=>{let Z=c8({input:process.stdin,output:process.stdout});Z.question($,(z)=>{Z.close();let X=z.trim().toLowerCase();Q(X==="y"||X==="yes")})})}async function s8($,Q,Z){let z=B6("hosted_publish");for(let B of z.notes)process.stderr.write(`${B}
...
`),process.stderr.write(`Use 'loki proof list' to see available proofs.
`),1;if(X)return s8(Q,q,e(I$(),Q,"proof.json"));if((await R(["gh","--version"],{timeoutMs:5000})).exitCode!==0)return process.stderr.write(`${T}gh CLI not found${V}
...
`);let B=`Loki Mode proof-of-run ${Q}`,G=["gh","gist","create",Y,"--desc",B];if(z!=="")G.push(z);let O=await R(G,{timeoutMs:60000});try{f8(H,{recursive:!0,force:!0})}catch{}if(O.exitCode!==0)return process.stderr.write(`${T}Failed to create gist${V}
...
`),1}}var l8;var T6=P(()=>{C();d();c();G6();l8=`${k}loki proof${V} - inspect and share proof-of-run artifacts
...
`});var L6={};b(L6,{runCrash:()=>q7});import{existsSync as A6,readdirSync as i8,readFileSync as e8}from"fs";import{join as w6}from"path";function L$($){return $===void 0||$===null?"-":String($)}function f$($,Q){return $.length>=Q?$:$+" ".repeat(Q-$.length)}function _6(){return w6(j(),"crash")}function I1(){let $=_6();if(!A6($))return[];try{return i8($,{withFileTypes:!0}).filter((Q)=>Q.isFile()&&Q.name.endsWith(".json")).map((Q)=>Q.name.slice(0,-5)).sort()}catch{return[]}}function Q7($){if($.length===0)return!1;if($.includes("/")||$.includes("\\"))return!1;if($.includes(".."))return!1;return!0}function u$($){if(!Q7($))return null;let Q=w6(_6(),`${$}.json`);if(!A6(Q))return null;try{return JSON.parse(e8(Q,"utf8"))}catch{return{}}}function Z7(){let $=I1();if($.length===0)return process.stdout.write(`${_}No crash reports found.${V} Nothing has been captured in .loki/crash/.
...
`),process.stdout.write(O6),2}}var $7="https://github.com/asklokesh/loki-mode/issues/new",O6;var P6=P(()=>{C();c();O6=`${k}loki crash${V} - inspect and manually submit local crash reports
...
`});var k6={};b(k6,{runWiki:()=>H7});import{existsSync as L1,readFileSync as j6}from"fs";import{join as P1,resolve as V7}from"path";function W7(){return P1(process.cwd(),".loki","wiki")}function U7($){let Q="";for(let X of $){if(X==="--help"||X==="-h")return process.stdout.write(`Usage: loki wiki show [section]
Sections: architecture, modules, data-flow
`),0;if(X.startsWith("-"))return process.stderr.write(`${T}Unknown option: ${X}${V}
`),1;Q=X}let Z=W7();if(!L1(Z))return process.stderr.write(`${_}No wiki found. Run 'loki wiki generate' first.${V}
...
`),process.stdout.write(F6),1}}var F6,J7;var x6=P(()=>{C();c();F6=`${k}loki wiki${V} - Auto-generated, cited codebase wiki + Q&A
...
`),0;let q=j$(Z,"state");s7(q,{recursive:!0}),A$(j$(q,`findings-${Q}.json`),{review_id:X.reviewId,iteration:Q,findings:X.findings});let K=await Promise.resolve().then(() => (l$(),g6)),W=0,J=0;if(process.env.LOKI_AUTO_LEARNINGS!=="0"){let H=X.findings.filter((B)=>B.severity==="Critical"||B.severity==="High"),Y=n6.fn??K.appendFromGateFailure;for(let B of H)try{await Y(Z,Q,B,{episodeBridge:null}),W+=1}catch(G){J+=1,process.stderr.write(`reflect: learning append failed for finding ${B.reviewer}/${B.severity}: ${G.message}
...
`),0;let J=new Set(["duplicate-code-path","file-exists","test-passes","grep-miss","out-of-scope"]),U=async(M)=>{let L=J.has(M.evidence.proofType);return{judge:M.judge,verdict:L?"APPROVE_OVERRIDE":"REJECT_OVERRIDE",reasoning:L?`[stub] proofType=${M.evidence.proofType} trusted`:`[stub] proofType=${M.evidence.proofType} requires manual review`}},H=await z.runOverrideCouncil(W,X,U);await z.recordOverrideOutcome(Z,Q,H,W);let Y=j$(Z,"quality","reviews");if(a7(Y))try{let M=r7(Y).filter((x)=>x.startsWith("review-")).sort(),L=M[M.length-1];if(L&&t7(j$(Y,L)).isDirectory())A$(j$(Y,L,`override-${Q}.json`),{review_id:K.reviewId,iteration:Q,approved_finding_ids:Array.from(H.approvedFindingIds),rejected_finding_ids:Array.from(H.rejectedFindingIds),votes:H.votes})}catch{}let B=H.approvedFindingIds.size,G=H.rejectedFindingIds.size;if(G===0&&B>0)process.stdout.write(`override: LIFTED -- ${B} approved, ${G} rejected
...
`,zZ;var s6=P(()=>{C();m$();n6={fn:null};zZ=x1});r$();function C1(){return process.stdout.write(`Loki Mode v${R$()}
`),0}d();c();C();import{readFileSync as HQ,existsSync as BQ}from"fs";import{resolve as GQ}from"path";var YQ=["claude","codex","cline","aider"];function h1(){let $=GQ(j(),"state","provider");if(!BQ($))return"";try{return HQ($,"utf-8").trim()}catch{return""}}function MQ($,Q){return $||Q||process.env.LOKI_PROVIDER||"claude"}function TQ($){let Q=h1(),Z=MQ($,Q);switch(process.stdout.write(`${k}Current Provider${V}
`),process.stdout.write(`
...
`),0}async function OQ(){let Q=h1()||process.env.LOKI_PROVIDER||"claude";process.stdout.write(`${k}Available Providers${V}
`),process.stdout.write(`
...
`),0}function AQ(){return process.stdout.write(`${k}Loki Mode Provider Management${V}
`),process.stdout.write(`
...
`),0;return process.stdout.write(Z.stdout),0}async function m1($){switch($[0]??"list"){case"list":case"ls":return PQ();case"index":return jQ($[1]==="rebuild");default:{let Z=J$(g,"autonomy","loki"),z=3600000,X=Bun.spawn({cmd:[Z,"memory",...$],stdin:"inherit",stdout:"inherit",stderr:"inherit",env:{...process.env,LOKI_LEGACY_BASH:"1"}}),q=setTimeout(()=>{try{X.kill("SIGKILL")}catch{}},3600000);try{return await X.exited}finally{clearTimeout(q)}}}}C();V$();d();import{resolve as FQ,join as RQ}from"path";import{existsSync as e$,readFileSync as kQ}from"fs";import{homedir as xQ}from"os";import{spawnSync as c1}from"child_process";var p1=3000;function EQ(){let $=(process.env.LOKI_TELEMETRY??"").toLowerCase();if($==="off")return!1;if(process.env.LOKI_TELEMETRY_DISABLED==="true")return!1;if(process.env.DO_NOT_TRACK==="1")return!1;let Q=!1,Z=!1;try{let z=RQ(xQ(),".loki","config");if(e$(z)){let X=kQ(z,"utf8");for(let q of X.split(`
`)){let K=q.replace(/\r$/,"");if(K==="TELEMETRY_DISABLED=true")Q=!0;if(K==="TELEMETRY_ENABLED=true")Z=!0}}}catch{}if(Q)return!1;if($==="on"||Z)return!0;return!1}var k$=!1;function NQ(){return FQ(g,"autonomy","lib","crash_capture.py")}function SQ($,Q){let Z=[$,"--error-class",Q.errorClass,"--message",Q.message];if(Q.stack!==void 0)Z.push("--stack",Q.stack);if(Q.rarvPhase!==void 0)Z.push("--rarv-phase",Q.rarvPhase);if(Q.exitCode!==void 0)Z.push("--exit-code",String(Q.exitCode));if(Q.frictionKind!==void 0)Z.push("--friction-kind",Q.frictionKind);return Z.push("--target-dir",Q.targetDir??process.cwd()),Z}function DQ(){if(e$("/opt/homebrew/bin/python3.12"))return"/opt/homebrew/bin/python3.12";for(let Q of["python3.12","python3"])try{let Z=c1("sh",["-c",`command -v ${Q}`],{timeout:p1,encoding:"utf8"});if(Z.status===0){let z=(Z.stdout||"").trim();if(z)return z}}catch{}return null}function g1($){try{if(!EQ())return;let Q=NQ();if(!e$(Q))return;let Z=DQ();if(!Z)return;let z=SQ(Q,$);c1(Z,z,{timeout:p1,stdio:"ignore"})}catch{}}function f1($,Q){if($ instanceof Error){let z={errorClass:$.name&&$.name.length>0?$.name:Q,message:$.message};if($.stack)z.stack=$.stack;return z}return{errorClass:Q,message:String($)}}var u1=!1;function l1(){if(u1)return;u1=!0,process.on("uncaughtException",($)=>{if(!k$){k$=!0;let Q=f1($,"UncaughtException");g1({errorClass:Q.errorClass,message:Q.message,...Q.stack!==void 0?{stack:Q.stack}:{},exitCode:1})}try{process.stderr.write(`${$&&$.stack||String($)}
