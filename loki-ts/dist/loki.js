...
`),!1}function E$($){if(!Number.isFinite($)||$<=0)return!1;try{return process.kill($,0),!0}catch{return!1}}function S$($){if(!y($))return null;try{let Q=W$($,"utf-8").trim();if(!Q)return null;let Z=Number.parseInt(Q,10);return Number.isFinite(Z)?Z:null}catch{return null}}function vQ($){let Q=[],Z=S$(D($,"loki.pid"));if(Z!==null&&E$(Z))Q.push(`global:${Z}`);let z=D($,"sessions");if(y(z)){let X=[];try{X=d1(z)}catch{X=[]}for(let q of X){let K=D(z,q);try{if(!o1(K).isDirectory())continue}catch{continue}let W=D(K,"loki.pid"),J=S$(W);if(J!==null&&E$(J))Q.push(`${q}:${J}`)}}if(y($)){let X=[];try{X=d1($)}catch{X=[]}for(let q of X){if(!q.startsWith("run-")||!q.endsWith(".pid"))continue;let K=D($,q);try{if(!o1(K).isFile())continue}catch{continue}let W=CQ(q,".pid").slice(4),J=S$(K);if(J!==null&&E$(J)){if(!Q.some((H)=>H.startsWith(`${W}:`)))Q.push(`${W}:${J}`)}}}return Q}async function s1($,Q){let Z=await F(["jq","-r",$,Q]);if(Z.exitCode!==0)return null;return Z.stdout.trim()}function t1($,Q){try{let Z=W$($,"utf-8"),X=JSON.parse(Z)[Q];if(typeof X==="number"){if(Q==="budget_used"){let q=Math.round(X*100)/100;if(Number.isInteger(q))return String(q);return String(q)}return String(X)}if(X===void 0||X===null)return"0";return String(X)}catch{return"0"}}function r1($,Q,Z){try{let z=W$($,"utf-8"),q=JSON.parse(z)[Q];if(typeof q==="number"&&Number.isFinite(q))return q;return Z}catch{return Z}}async function mQ(){let $=P();if(!await yQ())return 1;if(!y($))return process.stdout.write(`${R}Loki Mode Status${V}
...
`),process.stdout.write(`
`);let q=vQ($);if(q.length>0){process.stdout.write(`${N}Active Sessions: ${q.length}${V}
`);for(let Y of q){let T=Y.indexOf(":"),M=T>=0?Y.slice(0,T):Y,k=T>=0?Y.slice(T+1):"";if(M==="global")process.stdout.write(`  ${I}[global]${V} PID ${k}
`);else process.stdout.write(`  ${I}[#${M}]${V} PID ${k}
`)}process.stdout.write(`
...
`);let Y=await s1('.currentPhase // "unknown"',W);process.stdout.write(`${Y??"unknown"}
`)}let J=D($,"queue","pending.json");if(y(J)){let Y=await s1('if type == "array" then length elif .tasks then .tasks | length else 0 end',J);process.stdout.write(`${I}Pending Tasks:${V} ${Y??"0"}
`)}let U=D($,"metrics","budget.json");if(y(U)){let Y=t1(U,"budget_limit"),T=t1(U,"budget_used");if(Y!=="0"){process.stdout.write(`${I}Budget:${V} $${T} / $${Y}
`);let M=Math.trunc((Number.parseFloat(T)||0)*100),k=Number.parseFloat(Y),S=Number.isFinite(k)&&k!==0?Math.trunc(k*100):100,v=a1(M,S,"Budget");if(v!==null)process.stdout.write(`${v}
`)}else process.stdout.write(`${I}Cost:${V} $${T} (no limit)
`)}let H=D($,"state","context-usage.json");if(y(H)){let Y=r1(H,"window_size",200000),T=r1(H,"used_tokens",0),M=a1(T,Y,"Context");if(M!==null)process.stdout.write(`${M}
`)}let G=[D($,"dashboard","dashboard.pid"),D(bQ(),".loki","dashboard","dashboard.pid")].find((Y)=>y(Y))??"";if(G&&y(G)){let Y=S$(G);if(Y!==null&&E$(Y)){let T=D(G,".."),M=(u,w)=>{let x=D(T,u);try{return y(x)?W$(x,"utf-8").trim()||w:w}catch{return w}},k=M("scheme","http"),S=M("host","127.0.0.1"),v=M("port",process.env.LOKI_DASHBOARD_PORT||"57374");if(S==="0.0.0.0")S="127.0.0.1";process.stdout.write(`${I}Dashboard:${V} ${k}://${S}:${v}/
...
`),0}async function gQ($){let Q=D($,"state"),Z=fQ(Q),z=D(Q,"relevant-learnings.json"),X=D($,"escalations"),q=Z.length>0,K=y(z),W=y(X);if(!q&&!K&&!W)return;if(process.stdout.write(`
${I}Phase 1 artifacts:${V}
`),q){let J=Z[Z.length-1],U=i1(J);if(U&&Array.isArray(U.findings)){let H={Critical:0,High:0,Medium:0,Low:0};for(let G of U.findings){let Y=String(G.severity??"");if(Y in H)H[Y]=(H[Y]??0)+1}let B=Object.entries(H).filter(([,G])=>G>0).map(([G,Y])=>`${Y} ${G.toLowerCase()}`).join(", ");process.stdout.write(`  Findings (iter ${U.iteration??"?"}): ${B||"none"} -- ${U.findings.length} total
`)}}if(K){let J=i1(z);if(J&&Array.isArray(J.learnings)&&J.learnings.length>0){let U=new Map;for(let B of J.learnings){let G=String(B.trigger??"unknown");U.set(G,(U.get(G)??0)+1)}let H=[...U.entries()].sort((B,G)=>G[1]-B[1]).slice(0,3).map(([B,G])=>`${G} ${B}`).join(", ");process.stdout.write(`  Learnings: ${J.learnings.length} total (${H})
`)}}if(W){let J=0,U="";try{let B=(await import("fs")).readdirSync(X).filter((G)=>G.endsWith(".md"));if(J=B.length,B.length>0)B.sort(),U=B[B.length-1]??""}catch{}if(J>0)process.stdout.write(`  Escalations: ${J} handoff doc${J===1?"":"s"} (latest: ${U})
`)}}function fQ($){if(!y($))return[];try{return q$("fs").readdirSync($).filter((z)=>/^findings-\d+\.json$/.test(z)).sort((z,X)=>{let q=Number.parseInt(z.replace(/[^0-9]/g,""),10)||0,K=Number.parseInt(X.replace(/[^0-9]/g,""),10)||0;return q-K}).map((z)=>D($,z))}catch{return[]}}function i1($){try{let Q=q$("fs");return JSON.parse(Q.readFileSync($,"utf-8"))}catch{return null}}async function uQ(){let $=await Z$();if(!$)return process.stderr.write(`{"error": "Failed to generate JSON status. Ensure python3 is available."}
`),1;let Q=g,Z=P(),z=process.env.LOKI_DASHBOARD_PORT||"57374",X=process.env.LOKI_PROVIDER||"claude",q=await F([$,"-c",hQ,Q,Z,z,X],{timeoutMs:30000});if(q.exitCode!==0)return process.stderr.write(`{"error": "Failed to generate JSON status. Ensure python3 is available."}
...
print(json.dumps(result, indent=2))
`;var $0=L(()=>{d();V$();c();C()});var Z0={};b(Z0,{emitDeprecatedAlias:()=>Q1,deprecatedAliasShouldSuppress:()=>Q0});function Q0($){let Q=$[0];if(Q!==void 0&&lQ.has(Q))return!0;for(let Z of $)if(pQ.has(Z))return!0;return!1}function Q1($,Q,Z){if(Q0(Z))return;process.stderr.write(`note: 'loki ${$}' is now 'loki ${Q}'. The old form still works.
`)}var pQ,lQ;var Z1=L(()=>{pQ=new Set(["--json","-q","--quiet"]),lQ=new Set(["json","csv","timeline"])});var q0={};b(q0,{runStats:()=>tQ,computeStats:()=>K0});import{readdirSync as z0,readFileSync as dQ,statSync as X0}from"fs";import{join as t}from"path";function U$($){try{if(!X0($).isFile())return null;return JSON.parse(dQ($,"utf-8"))}catch{return null}}function K1($){try{return X0($).isDirectory()}catch{return!1}}function oQ($){if(!K1($))return[];try{let Q=z0($).filter((Z)=>Z.startsWith("iteration-")&&Z.endsWith(".json"));return Q.sort(),Q.map((Z)=>t($,Z))}catch{return[]}}function H$($){return Math.trunc($).toLocaleString("en-US")}function z1($){let Q=Math.trunc($);if(Q<60)return`${Q}s`;let Z=Math.trunc(Q/3600),z=Math.trunc(Q%3600/60),X=Q%60;if(Z>0)return`${Z}h ${String(z).padStart(2,"0")}m`;return`${z}m ${String(X).padStart(2,"0")}s`}function r($,Q=0){let Z=Math.pow(10,Q);return Math.round($*Z)/Z}function G$($,Q){return $.toFixed(Q)}function X1($,Q){return $.length>=Q?$:$+" ".repeat(Q-$.length)}function nQ($){let Q="N/A",Z=0,z=U$(t($,"state","orchestrator.json"));if(z&&typeof z==="object"){if(typeof z.currentPhase==="string")Q=z.currentPhase;if(typeof z.currentIteration==="number")Z=z.currentIteration}let X=t($,"metrics","efficiency"),q=oQ(X),K=[];for(let j of q){let E=U$(j);if(E&&typeof E==="object")K.push(E)}if(K.length>0)Z=Math.max(Z,K.length);let W=K.reduce((j,E)=>j+(E.input_tokens??0),0),J=K.reduce((j,E)=>j+(E.output_tokens??0),0),U=W+J,H=K.reduce((j,E)=>j+(E.cost_usd??0),0),B=K.reduce((j,E)=>j+(E.duration_seconds??0),0),G=0,Y=0,T=U$(t($,"metrics","budget.json"));if(T&&typeof T==="object"){if(typeof T.budget_limit==="number")G=T.budget_limit;if(typeof T.budget_used==="number")Y=T.budget_used}let M=0,k=0,S=U$(t($,"state","quality-gates.json"));if(S&&typeof S==="object"){if(Array.isArray(S)){for(let j of S)if(k+=1,j===!0)M+=1;else if(j&&typeof j==="object"){let E=j;if(E.passed===!0||E.status==="passed")M+=1}}else for(let j of Object.values(S))if(typeof j==="boolean"){if(k+=1,j)M+=1}else if(j&&typeof j==="object"){k+=1;let E=j;if(E.passed===!0||E.status==="passed")M+=1}}let v={},u=U$(t($,"quality","gate-failure-count.json"));if(u&&typeof u==="object"&&!Array.isArray(u)){let j={};for(let[E,l]of Object.entries(u))if(typeof l==="number")j[E]=l;v=j}let w=0,x=0,n=0,n$=t($,"quality");if(K1(n$)){let j=[];try{j=z0(n$)}catch{j=[]}for(let E of j){if(!E.endsWith(".json")||E==="gate-failure-count.json")continue;let l=U$(t(n$,E));if(!l||typeof l!=="object")continue;if(!(("verdict"in l)||("approved"in l)||("reviewers"in l)))continue;w+=1;let S1=(l.verdict??"").toString().toLowerCase();if(l.approved===!0||["approved","approve","pass"].includes(S1))x+=1;else if(["revision","revise","changes_requested","reject"].includes(S1))n+=1}}return{phase:Q,iterationCount:Z,iterations:K,totalInput:W,totalOutput:J,totalTokens:U,totalCost:H,totalDuration:B,budgetLimit:G,budgetUsed:Y,gatesPassed:M,gatesTotal:k,gateFailures:v,reviewsTotal:w,reviewsApproved:x,reviewsRevision:n}}function aQ($,Q){let Z=$.iterationCount,z={session:{iterations:Z,duration_seconds:$.totalDuration,phase:$.phase},tokens:{input:$.totalInput,output:$.totalOutput,total:$.totalTokens,cost_usd:r($.totalCost,2)},quality:{gates_passed:$.gatesPassed,gates_total:$.gatesTotal,reviews_total:$.reviewsTotal,reviews_approved:$.reviewsApproved,reviews_revision:$.reviewsRevision,gate_failures:$.gateFailures},efficiency:{avg_tokens_per_iteration:Z>0?r($.totalTokens/Z,0):0,avg_cost_per_iteration:Z>0?r($.totalCost/Z,2):0,avg_duration_per_iteration:Z>0?r($.totalDuration/Z,1):0},budget:{used:r($.budgetUsed,2),limit:$.budgetLimit,percent:$.budgetLimit>0?r($.budgetUsed/$.budgetLimit*100,1):0}};if(Q)z.iterations=$.iterations.map((K,W)=>({number:W+1,input_tokens:K.input_tokens??0,output_tokens:K.output_tokens??0,cost_usd:r(K.cost_usd??0,2),duration_seconds:K.duration_seconds??0}));let X=JSON.stringify(z,null,2);function q(K,W){if(!W)return;let J=new RegExp(`("${K}": )(-?\\d+)(,?)$`,"m");X=X.replace(J,(U,H,B,G)=>`${H}${B}.0${G}`)}if(q("avg_duration_per_iteration",Z>0&&Number.isInteger(z.efficiency.avg_duration_per_iteration)),q("percent",$.budgetLimit>0&&Number.isInteger(z.budget.percent)),q("cost_usd",Z>0&&Number.isInteger(z.tokens.cost_usd)),Q)X=X.replace(/("cost_usd": )(-?\d+)(,?)$/gm,(K,W,J,U)=>`${W}${J}.0${U}`);return X}function sQ($,Q){let Z=[];if(Z.push("Loki Mode Session Statistics"),Z.push("============================"),Z.push(""),Z.push("Session"),Z.push(`  Iterations completed: ${$.iterationCount}`),Z.push(`  Duration: ${z1($.totalDuration)}`),Z.push(`  Current phase: ${$.phase}`),Z.push(""),Z.push("Token Usage"),$.iterations.length>0)Z.push(`  Input tokens:  ${H$($.totalInput)}`),Z.push(`  Output tokens: ${H$($.totalOutput)}`),Z.push(`  Total tokens:  ${H$($.totalTokens)}`),Z.push(`  Estimated cost: $${G$($.totalCost,2)}`);else Z.push("  N/A (no iteration metrics found)");if(Z.push(""),Z.push("Quality Gates"),$.gatesTotal>0){let z=Math.round($.gatesPassed/$.gatesTotal*100);Z.push(`  Gates passed: ${$.gatesPassed}/${$.gatesTotal} (${z}%)`)}else Z.push("  Gates passed: N/A");if($.reviewsTotal>0){let z=[];if($.reviewsApproved>0)z.push(`${$.reviewsApproved} approved`);if($.reviewsRevision>0)z.push(`${$.reviewsRevision} revision requested`);let X=z.length>0?z.join(", "):"N/A";Z.push(`  Code reviews: ${$.reviewsTotal} (${X})`)}if(Object.keys($.gateFailures).length>0){let z=Object.entries($.gateFailures).filter(([,X])=>X>0).map(([X,q])=>`${X} (${q})`);if(z.length>0)Z.push(`  Gate failures: ${z.join(", ")}`)}if(Z.push(""),Z.push("Efficiency"),$.iterationCount>0&&$.iterations.length>0){let z=Math.round($.totalTokens/$.iterationCount),X=$.totalCost/$.iterationCount,q=$.totalDuration/$.iterationCount;Z.push(`  Avg tokens/iteration: ${H$(z)}`),Z.push(`  Avg cost/iteration: $${G$(X,2)}`),Z.push(`  Avg duration/iteration: ${z1(q)}`)}else Z.push("  N/A (no iteration metrics found)");if(Z.push(""),Z.push("Budget"),$.budgetLimit>0){let z=r($.budgetUsed/$.budgetLimit*100,1),X=Number.isInteger(z)?`${z}.0`:`${z}`;Z.push(`  Used: $${G$($.budgetUsed,2)} / $${G$($.budgetLimit,2)} (${X}%)`)}else if($.budgetUsed>0)Z.push(`  Used: $${G$($.budgetUsed,2)} (no limit set)`);else Z.push("  N/A");if(Q&&$.iterations.length>0)Z.push(""),Z.push("Per-Iteration Breakdown"),$.iterations.forEach((z,X)=>{let q=X+1,K=X1(H$(z.input_tokens??0),10),W=X1(H$(z.output_tokens??0),10),J=z.cost_usd??0,U=z1(z.duration_seconds??0),H=X1(`${q}`,3);Z.push(`  #${H} input: ${K} output: ${W} cost: $${G$(J,2)}  time: ${U}`)});return Z.join(`
`)}function K0($){let Q=!1,Z=!1;for(let K of $)if(K==="--json")Q=!0;else if(K==="--efficiency")Z=!0;let z=P();if(!K1(z)){if(Q)return{exitCode:0,stdout:'{"error": "No active session"}'};return{exitCode:0,stdout:`${_}No active session found.${V}
Start a session with: loki start <prd>`}}let X=nQ(z);return{exitCode:0,stdout:Q?aQ(X,Z):sQ(X,Z)}}async function tQ($){let{emitDeprecatedAlias:Q}=await Promise.resolve().then(() => (Z1(),Z0));Q("stats","report session",$);let Z=K0($);return console.log(Z.stdout),Z.exitCode}var V0=L(()=>{C();c()});var T0={};b(T0,{runDoctor:()=>U3,pythonImportOk:()=>W1,httpReachable:()=>V1,checkTool:()=>G0,checkSkills:()=>B0,checkDisk:()=>J1,buildDoctorJson:()=>M0,_setPythonImportOkForTest:()=>z3});import{existsSync as J0,lstatSync as rQ,readlinkSync as iQ,statfsSync as eQ}from"fs";import{spawnSync as $3}from"child_process";import{homedir as W0}from"os";import{resolve as q1}from"path";function Z3($){let Q=$.match(Q3);return Q?Q[1]:null}async function U0($){try{let Q=await F([$,"--version"],{timeoutMs:5000}),Z=(Q.stdout||Q.stderr||"").trim();return Z3(Z)}catch{return null}}function H0($,Q){let Z=$.split(".").map((X)=>parseInt(X,10)),z=Q.split(".").map((X)=>parseInt(X,10));while(Z.length<2)Z.push(0);while(z.length<2)z.push(0);for(let X=0;X<2;X++){let q=Z[X]??0,K=z[X]??0;if(Number.isNaN(q)||Number.isNaN(K))return 0;if(q!==K)return q-K}return 0}async function G0($,Q,Z,z=null){let X=await f(Q),q=X!==null,K=q?await U0(Q):null,W="pass";if(!q)W=Z==="required"?"fail":"warn";else if(z&&K){if(H0(K,z)<0)W=Z==="required"?"fail":"warn"}return{name:$,command:Q,found:q,version:K,required:Z,min_version:z,status:W,path:X}}function J1(){let $=null;try{let Z=eQ(W0()),z=Number(Z.bavail)*Number(Z.bsize);$=Math.round(z/1073741824*10)/10}catch{$=null}let Q="pass";if($!==null){if($<1)Q="fail";else if($<5)Q="warn"}return{available_gb:$,status:Q}}async function V1($,Q=2000){try{return(await fetch($,{signal:AbortSignal.timeout(Q)})).ok}catch{return!1}}async function W1($,Q=!1){let Z=`import ${$}`,z=Q?30000:5000;if(!Q)return(await z$(Z,{timeoutMs:z})).exitCode===0;let X=await Z$();if(!X)return!1;return(await F([X,"-c",Z],{timeoutMs:z})).exitCode===0}function z3($){C$.fn=$??W1}function B0(){let $=W0();return X3.map(({name:Q,dir:Z})=>{let z=q1($,Z),X=z,q=q1(z,"SKILL.md");if(J0(q))return{name:Q,path:X,status:"pass",detail:""};try{if(rQ(z).isSymbolicLink()){let W="unknown";try{W=iQ(z)}catch{}return{name:Q,path:X,status:"fail",detail:`(broken symlink -> ${W})`}}}catch{}return{name:Q,path:X,status:"warn",detail:"(not found - run 'loki setup-skill')"}})}async function Y0(){return Promise.all(K3.map(async($)=>{return{...await G0($.jsonName,$.cmd,$.required,$.min??null),displayName:$.displayName}}))}async function q3(){let Q=await f("sentrux")!==null,Z=Q?await U0("sentrux"):null;return{found:Q,version:Z,status:Q?"pass":"warn",required:"optional"}}async function V3(){let{openSync:$,statSync:Q,readSync:Z,closeSync:z,existsSync:X}=await import("fs"),{join:q}=await import("path"),K=65536,W=process.env.LOKI_DIR??".loki",J=q(W,"memory",".errors.log"),U=[],H=!1;try{if(X(J)){H=!0;let B=Q(J).size,G=Math.max(0,B-65536),Y=B-G,T=Buffer.alloc(Y),M=$(J,"r");try{Z(M,T,0,Y,G)}finally{z(M)}let S=T.toString("utf-8").split(`
`);if(G>0&&S.length>0)S=S.slice(1);S=S.map((v)=>v.trim()).filter((v)=>v.length>0),U=S.slice(-5)}}catch{U=[]}return{errors_log_path:H?J:null,recent_errors:U,recent_error_count:U.length,status:U.length===0?"pass":"warn"}}async function M0(){let Q=(await Y0()).map(({displayName:J,...U})=>U),Z=J1(),z=await q3(),X=await V3(),q=0,K=0,W=0;for(let J of Q)if(J.status==="pass")q++;else if(J.status==="fail")K++;else W++;if(Z.status==="pass")q++;else if(Z.status==="fail")K++;else W++;return{loki_mode_version:R$(),checks:Q,disk:Z,sentrux:z,memory:X,summary:{passed:q,failed:K,warnings:W,ok:K===0}}}function A($){switch($){case"pass":return`${N}PASS${V}`;case"fail":return`${O}FAIL${V}`;case"warn":return`${_}WARN${V}`}}function N$($){let Q=$.version?` (v${$.version})`:"",Z=$.displayName;if(!$.found){let z=$.required==="required"?"not found":$.required==="recommended"?"not found (recommended)":"not found (optional)";return`  ${A($.status)}  ${Z} - ${z}`}if($.min_version&&$.version&&H0($.version,$.min_version)<0){let z=$.required==="required"?"requires":"recommended";return`  ${A($.status)}  ${Z}${Q} - ${z} >= ${$.min_version}`}return`  ${A($.status)}  ${Z}${Q}`}function D$($,Q){if(Q==="pass")$.pass++;else if(Q==="fail")$.fail++;else $.warn++}function J3(){process.stdout.write(`${R}loki doctor${V} - Check system prerequisites
...
`);for(let w of["node","python3","jq","git","curl"]){let x=Z.get(w);process.stdout.write(N$(x)+`
`),D$($,x.status)}process.stdout.write(`
`),process.stdout.write(`${I}AI Providers:${V}
`);let z=["claude","codex","cline","aider"],X={claude:"npm install -g @anthropic-ai/claude-code",codex:"npm install -g @openai/codex",cline:"npm install -g cline",aider:"pip install aider-chat"},q=!1;for(let w of z){let x=Z.get(w);if(process.stdout.write(N$(x)+`
`),!x.found&&X[w])process.stderr.write(`         ${_}Install: ${X[w]}${V}
`);if(D$($,x.status),x.found)q=!0}if(!q){if(process.stdout.write(`  ${A("fail")}  No AI provider CLI installed -- at least one is required
`),process.stdout.write(`         ${_}Install: npm install -g @anthropic-ai/claude-code${V}
`),$.fail++,process.stdout.isTTY){let w=q1(g,"autonomy/provider-offer.sh");if(J0(w))$3("bash",[w,"report"],{stdio:"inherit"})}}process.stdout.write(`
...
`);for(let w of B0())if(w.status==="pass")process.stdout.write(`  ${A("pass")}  ${w.name}  ${h}${w.path}${V}
...
`);let[U,H,B]=await Promise.all([C$.fn("mcp"),C$.fn("numpy",!0),C$.fn("sentence_transformers",!0)]);if(U)process.stdout.write(`  ${A("pass")}  MCP SDK (Python)
...
`),$.warn++;if(await V1("http://localhost:8100/api/v2/heartbeat"))process.stdout.write(`  ${A("pass")}  ChromaDB server (port 8100)
`),$.pass++;else process.stdout.write(`  ${A("warn")}  ChromaDB - not running (docker start loki-chroma)
`),$.warn++;{let w=["pyright-langserver","pylsp","typescript-language-server","gopls","rust-analyzer","jdtls"],x=[];for(let n of w)if(await f(n))x.push(n);if(x.length>0)process.stdout.write(`  ${A("pass")}  LSP servers detected (${x.length}): ${x.join(", ")}
...
`),$.warn++}let G=process.env.LOKI_MIROFISH_URL;if(G)if(await V1(`${G}/health`))process.stdout.write(`  ${A("pass")}  MiroFish server (${G})
...
`),$.warn++;if(await f("sentrux")){let w="unknown";try{let n=(await F(["sentrux","--version"],{timeoutMs:2000})).stdout.split(/\s+/).filter(Boolean).pop();if(n)w=n.replace(/^v/,"")}catch{}process.stdout.write(`  ${A("pass")}  sentrux ${w} (architectural drift gate: loki sentrux help)
...
`);let Y=Z.get("bash");process.stdout.write(N$(Y)+`
`),D$($,Y.status);let T=Z.get("bun");if(T)process.stdout.write(N$(T)+`
`),D$($,T.status);let M=J1(),k=M.available_gb===null?null:Math.floor(M.available_gb);if(k===null)process.stdout.write(`  ${A("warn")}  Disk space: unable to determine
`),$.warn++;else if(M.status==="fail")process.stdout.write(`  ${A("fail")}  Disk space: ${k}GB available (need >= 1GB)
`),$.fail++;else if(M.status==="warn")process.stdout.write(`  ${A("warn")}  Disk space: ${k}GB available (low)
`),$.warn++;else process.stdout.write(`  ${A("pass")}  Disk space: ${k}GB available
`),$.pass++;process.stdout.write(`
`),process.stdout.write(`${I}Runtime route:${V}
`);let S=process.versions.bun!==void 0,v=process.argv[0]??"(unknown)";if(process.stdout.write(`  ${A("pass")}  Active runtime: ${S?"Bun":"Node"} (${v})
...
`),process.stdout.write(`${R}Summary:${V} ${N}${$.pass} passed${V}, ${O}${$.fail} failed${V}, ${_}${$.warn} warnings${V}
...
`),Q}catch(Z){if(Q!==null){try{B1(Q)}catch{}try{p0($)}catch{}}if(Z?.code==="EEXIST")return null;throw Z}}function H8($,Q){let Z;try{Z=$8($)}catch{return!0}if(Z.isSymbolicLink())try{return p0($),!0}catch{return!1}let z;try{z=u0($,"r")}catch{return!0}try{let X=e3(z);if(Date.now()-X.mtimeMs<Q)return!1;let K=NaN;try{let W=Buffer.alloc(64),J=Q8(z,W,0,64,0);K=Number.parseInt(W.subarray(0,J).toString("utf-8").trim(),10)}catch{}if(Number.isFinite(K)&&W8(K))return!1;try{if(z8($).mtimeMs>X.mtimeMs)return!1}catch{return!0}try{c0($,{force:!0})}catch{}return!0}finally{try{B1(z)}catch{}}}function w$($,Q,Z={}){let z=Z.timeoutMs??1e4,X=Z.pollMs??25,q=Z.staleMs??30000,K=J8($),W=Date.now()+z,J=null,U=0,H=new Int32Array(new SharedArrayBuffer(4));while(J===null){if(J=U8(K),J!==null)break;if(Date.now()>W)throw Error(`withFileLockSync: timed out after ${z}ms acquiring ${K}`);if(H8(K,q))continue;let B=Math.min(X*2**Math.min(U,4),V8);U+=1,Atomics.wait(H,0,0,B)}try{return Q()}finally{try{B1(J)}catch{}try{c0(K,{force:!0})}catch{}}}var q8=0,v$,V8=50;var m$=L(()=>{v$=new Map});import{existsSync as K$,mkdirSync as B$,copyFileSync as s0,readFileSync as T1,readdirSync as G8,statSync as B8,writeFileSync as Y8,renameSync as t0,appendFileSync as r0,rmSync as M8}from"fs";import{join as m,dirname as g$}from"path";function O8($){let Q=n0.then($,$);return n0=Q.catch((Z)=>{console.warn("[checkpoint] serialized op rejected:",Z);return}),Q}function o($){return m($,"state","checkpoints")}function i0($){return m(o($),"index.jsonl")}async function A8($){let Q=await F(["git","rev-parse","HEAD"],{cwd:$,timeoutMs:5000});if(Q.exitCode!==0)return"no-git";return Q.stdout.trim()||"no-git"}async function w8($){let Q=await F(["git","branch","--show-current"],{cwd:$,timeoutMs:5000});if(Q.exitCode!==0)return"unknown";return Q.stdout.trim()||"unknown"}async function _8($){let Q=await F(["git","diff","--quiet"],{cwd:$,timeoutMs:5000}),Z=await F(["git","diff","--cached","--quiet"],{cwd:$,timeoutMs:5000}),z=Q.exitCode===1,X=Z.exitCode===1;return z||X}function I8($){let Q=m($,"state","orchestrator.json");if(!K$(Q))return"unknown";try{let z=JSON.parse(T1(Q,"utf-8")).currentPhase;return typeof z==="string"&&z.length>0?z:"unknown"}catch{return"unknown"}}function P8($,Q){for(let Z of L8){let z=m($,Z);if(!K$(z))continue;let X=m(Q,Z);B$(g$(X),{recursive:!0});try{s0(z,X)}catch{}}}function $6($,Q){B$(g$($),{recursive:!0});let Z=`${$}.tmp.${process.pid}.${++e0}`;Y8(Z,Q),t0(Z,$)}function j8($){return JSON.stringify($,null,2)}function Q6($){return`{${[`"id": ${JSON.stringify($.id)}`,`"ts": ${JSON.stringify($.ts)}`,`"iter": ${JSON.stringify($.iter)}`,`"task": ${JSON.stringify($.task)}`,`"sha": ${JSON.stringify($.sha)}`].join(", ")}}`}async function F8($){return O8(()=>R8($))}async function R8($){let Q=$.lokiDirOverride??P(),Z=process.cwd(),z=o(Q);if(B$(z,{recursive:!0}),!$.forceCreate){if(!await _8(Z))return{created:!1,reason:"no uncommitted changes"}}let X=await A8(Z),q=await w8(Z),K=$.iteration??Number.parseInt(process.env.ITERATION_COUNT??"0",10),W=$.epochOverride??Math.floor(Date.now()/1000),J=`cp-${K}-${W}`,U=m(z,J);B$(U,{recursive:!0}),P8(Q,U);let H=new Date().toISOString().replace(/\.\d{3}Z$/,"Z"),B=($.taskDescription??"task completed").slice(0,T8),G=$.provider??process.env.PROVIDER_NAME??"claude",Y={id:J,timestamp:H,iteration:K,task_id:$.taskId??"unknown",task_description:B,git_sha:X,git_branch:q,provider:G,phase:I8(Q)};$6(m(U,"metadata.json"),j8(Y));let T={id:Y.id,ts:Y.timestamp,iter:Y.iteration,task:Y.task_description,sha:Y.git_sha},M=i0(Q);return w$(M,()=>{r0(M,`${Q6(T)}
`)}),k8(Q),{created:!0,id:J,metadata:Y,dir:U}}function O1($){let Q=o($);if(!K$(Q))return[];return G8(Q).filter((Z)=>Z.startsWith("cp-")).filter((Z)=>{try{return B8(m(Q,Z)).isDirectory()}catch{return!1}})}function A1($){return[...$].sort((Q,Z)=>{let z=a0(Q),X=a0(Z);return z-X})}function a0($){let Q=$.split("-");if(Q.length<3)return 0;let Z=Q[Q.length-1],z=Number.parseInt(Z??"0",10);return Number.isFinite(z)?z:0}function k8($){let Q=O1($);if(Q.length<=o0)return;let Z=A1(Q),z=Z.slice(0,Z.length-o0);for(let X of z)try{M8(m(o($),X),{recursive:!0,force:!0})}catch{}x8($)}function x8($){let Q=A1(O1($)),Z=[];for(let q of Q){let K=m(o($),q,"metadata.json"),W=m(o($),q);if(!K$(K)){Y1($,W,"missing_field","metadata.json");continue}try{let J=JSON.parse(T1(K,"utf-8")),U=z6(J,K);if(!U.ok){Y1($,W,U.reason,U.field);continue}let H=U.value;Z.push(Q6({id:H.id,ts:H.timestamp,iter:H.iteration,task:H.task_description??"",sha:H.git_sha}))}catch{Y1($,W,"invalid_type","metadata.json")}}let z=i0($),X=Z.length>0?`${Z.join(`
`)}
`:"";$6(z,X)}function Y1($,Q,Z,z){let X=m($,"events.jsonl"),q={timestamp:new Date().toISOString(),type:"checkpoint.metadata.dropped",checkpoint_dir:Q,reason:Z,field:z};try{B$(g$(X),{recursive:!0}),w$(X,()=>{r0(X,`${JSON.stringify(q)}
`)})}catch{}}function w1($){let Q=$??P(),Z=A1(O1(Q)),z=[];for(let X of Z){let q=Z6(Q,X);if(q)z.push(q)}return z}function Z6($,Q){let Z=m(o($),Q,"metadata.json");if(!K$(Z))return null;try{let z=JSON.parse(T1(Z,"utf-8"));return E8(z,Z)}catch{return null}}function E8($,Q){let Z=z6($,Q);return Z.ok?Z.value:null}function z6($,Q){if($===null||typeof $!=="object")return console.warn(`[checkpoint] invalid metadata at ${Q}: not an object`),{ok:!1,reason:"invalid_type",field:"<root>"};let Z=$,z=["id","timestamp","task_id","task_description","git_sha","git_branch","provider","phase"];for(let X of z){if(!(X in Z))return console.warn(`[checkpoint] invalid metadata at ${Q}: field "${X}" missing`),{ok:!1,reason:"missing_field",field:X};if(typeof Z[X]!=="string")return console.warn(`[checkpoint] invalid metadata at ${Q}: field "${X}" not a string`),{ok:!1,reason:"invalid_type",field:X}}if(!Object.prototype.hasOwnProperty.call(Z,"iteration"))return console.warn(`[checkpoint] invalid metadata at ${Q}: field "iteration" missing`),{ok:!1,reason:"missing_field",field:"iteration"};if(typeof Z.iteration!=="number"||!Number.isFinite(Z.iteration))return console.warn(`[checkpoint] invalid metadata at ${Q}: field "iteration" not a finite number`),{ok:!1,reason:"invalid_type",field:"iteration"};for(let X of N8){let q=Z[X];if(S8.test(q))return console.warn(`[checkpoint] invalid metadata at ${Q}: field "${X}" contains control characters`),{ok:!1,reason:"control_chars",field:X}}return{ok:!0,value:{id:Z.id,timestamp:Z.timestamp,iteration:Z.iteration,task_id:Z.task_id,task_description:Z.task_description,git_sha:Z.git_sha,git_branch:Z.git_branch,provider:Z.provider,phase:Z.phase}}}function _1($,Q){if(!D8.test($))throw new X6($);let Z=Q??P(),z=m(o(Z),$);if(!K$(z))throw new M1($);let X=Z6(Z,$);if(!X)throw new M1($);return X}function K6($,Q){let Z=_1($,Q),z=Q??P(),X=m(o(z),$),q=[];for(let K of C8){let W=m(X,K);if(!K$(W))continue;q.push({from:W,to:m(z,K)})}return{id:$,metadata:Z,restore:q}}function b8($){let Q=[],Z=0;for(let z of $.restore)try{B$(g$(z.to),{recursive:!0});let X=`${z.to}.tmp.${process.pid}.${++e0}`;s0(z.from,X),t0(X,z.to),Z+=1}catch(X){Q.push(`${z.from} -> ${z.to}: ${X.message}`)}return{restored:Z,errors:Q}}async function q6($,Q,Z=!1){let z=null;try{let q=await F8({taskDescription:`pre-rollback snapshot (before restoring ${$.id})`,taskId:"rollback",forceCreate:!0,lokiDirOverride:Q});if(q.created)z=q.id}catch(q){let K=q instanceof Error?q.message:String(q);if(!Z)throw Error("pre-rollback snapshot failed ("+K+"); aborting rollback to preserve current state. Re-run with force to roll back anyway without a safety snapshot.");console.warn("[checkpoint] pre-rollback snapshot failed; proceeding due to force:",K)}let X=b8($);return{preRollbackSnapshotId:z,restored:X.restored,errors:X.errors}}var o0=50,T8=200,n0,L8,e0=0,S8,N8,D8,M1,X6,C8;var V6=L(()=>{C();d();m$();n0=Promise.resolve();L8=["state/orchestrator.json","autonomy-state.json","queue/pending.json","queue/completed.json","queue/in-progress.json","queue/current-task.json","CONTINUITY.md"];S8=/[\x00-\x08\x0a-\x1f\x7f-\x9f]/,N8=["id","task_id","git_sha","git_branch","provider","phase"];D8=/^[a-zA-Z0-9_-]+$/;M1=class M1 extends Error{id;constructor($){super(`Checkpoint not found: ${$}`);this.id=$;this.name="CheckpointNotFoundError"}};X6=class X6 extends Error{id;constructor($){super(`Invalid checkpoint ID: must be alphanumeric, hyphens, underscores only (got: ${$})`);this.id=$;this.name="InvalidCheckpointIdError"}};C8=["state/orchestrator.json","queue/pending.json","queue/completed.json","queue/in-progress.json","queue/current-task.json","CONTINUITY.md"]});var U6={};b(U6,{runRollback:()=>h8});async function h8($){let Q=$[0],Z=$.slice(1);if(Q===void 0||Q==="help"||Q==="--help"||Q==="-h")return process.stdout.write(J6),Q===void 0?1:0;switch(Q){case"list":{let z=[...w1()].reverse();if(z.length===0)return process.stdout.write(`${_}No checkpoints found.${V}
...
`),2;return await W6(z,Z.includes("--force"))}case"latest":{let z=w1(),X=z[z.length-1];if(!X)return process.stderr.write(`${O}No checkpoints found to roll back to.${V}
...
`),process.stderr.write(J6),2}}async function W6($,Q=!1){let Z;try{Z=K6($)}catch(X){return process.stderr.write(`${O}Cannot plan rollback:${V} ${X.message}
`),1}if(Z.restore.length===0)return process.stdout.write(`${_}Checkpoint ${$} has no restorable state files; nothing to do.${V}
`),0;let z;try{z=await q6(Z,void 0,Q)}catch(X){return process.stderr.write(`${O}Rollback aborted:${V} ${X.message}
...
`),1}if(process.stdout.write(`${N}Rolled back ${z.restored}/${Z.restore.length} state files from ${$}.${V}
...
`;var H6=L(()=>{V6();c()});function y8(){return process.env.LOKI_TIER||"oss"}function G6($){let Q=y8();if(Q==="oss")return{allowed:!0,notes:[]};if(!process.env.LOKI_LICENSE_KEY)return{allowed:!1,notes:[`${_}LOKI_TIER='${Q}' requested but no LOKI_LICENSE_KEY set.${V}`,`Hosted/enterprise license verification is not available yet (capability: ${$}).`,"OSS users: leave LOKI_TIER unset (or 'oss') -- everything stays free."]};return{allowed:!0,notes:[`${_}LOKI_LICENSE_KEY set but the verification backend is not available yet (R9 seam).${V}`]}}var B6=L(()=>{c()});var M6={};b(M6,{runProof:()=>r8});import{existsSync as _$,readdirSync as v8,readFileSync as Y6,mkdtempSync as m8,copyFileSync as g8,rmSync as f8}from"fs";import{join as e}from"path";import{tmpdir as u8}from"os";import{createInterface as c8}from"readline";import{readFile as p8}from"fs/promises";function $$($){return $&&typeof $==="object"?$:{}}function p($){return $===void 0||$===null?"-":String($)}function I$(){return e(P(),"proofs")}function I1($){let Q=e(I$(),$,"proof.json");if(!_$(Q))return null;try{return JSON.parse(Y6(Q,"utf8"))}catch{return{}}}function i($,Q){return $.length>=Q?$:$+" ".repeat(Q-$.length)}function d8(){let $=I$();if(!_$($))return process.stdout.write(`${_}No proofs found.${V} Run 'loki start' to generate one.
...
`);return 0}function o8($){if(!$)return process.stderr.write(`${O}Missing proof id.${V} Use 'loki proof list'.
`),2;let Q=I1($);if(Q===null)return process.stderr.write(`${O}Proof not found: ${$}${V}
`),process.stderr.write(`Use 'loki proof list' to see available proofs.
`),1;return process.stdout.write(`${JSON.stringify(Q,null,2)}
`),0}async function n8($){if(!$)return process.stderr.write(`${O}Missing proof id.${V} Use 'loki proof list'.
`),2;let Q=e(I$(),$,"index.html");if(!_$(Q))return process.stderr.write(`${O}Proof page not found: ${$}/index.html${V}
`),process.stderr.write(`Use 'loki proof list' to see available proofs.
`),1;process.stdout.write(`${N}Opening proof: ${Q}${V}
...
`),1;let q=I1($);if(q){if($$(q.redaction).applied!==!0)return process.stderr.write(`${O}Refusing to publish: proof redaction was not confirmed applied.${V}
...
`);let K;try{K=await p8(Q)}catch{return process.stderr.write(`${O}Could not read proof page: ${Q}${V}
`),1}let W={"Content-Type":"text/html","X-Loki-Proof-Id":$},J=process.env.LOKI_LICENSE_KEY||"";if(J)W.Authorization=`Bearer ${J}`;let U;try{U=await fetch(X,{method:"POST",headers:W,body:new Uint8Array(K)})}catch(G){return process.stderr.write(`${O}Failed to reach hosted endpoint: ${String(G.message||G)}${V}
`),process.stderr.write(`Check LOKI_HOSTED_ENDPOINT or publish to a gist: loki proof share ${$}
`),1}let H=await U.text();if(!U.ok){if(process.stderr.write(`${O}Hosted endpoint returned HTTP ${U.status}.${V}
...
`),1}let B="";try{let G=JSON.parse(H);if(G&&typeof G==="object"){let Y=G.url??G.public_url;if(typeof Y==="string")B=Y}}catch{}if(B)process.stdout.write(`${N}Published: ${B}${V}
`);else process.stdout.write(`${N}Published to ${X} (HTTP ${U.status}).${V}
`),process.stdout.write(`The endpoint did not return a 'url' field; check your endpoint's response.
`);return 0}async function t8($){let Q="",Z=!1,z="--public",X=!1;for(let M of $)if(M==="--yes"||M==="-y")Z=!0;else if(M==="--private")z="";else if(M==="--public")z="--public";else if(M==="--hosted")X=!0;else if(M.startsWith("-"))return process.stderr.write(`${O}Unknown option: ${M}${V}
`),1;else Q=M;if(!Q)return process.stderr.write(`${O}Missing proof id.${V} Use 'loki proof list'.
`),2;let q=e(I$(),Q,"index.html");if(!_$(q))return process.stderr.write(`${O}Proof page not found: ${Q}/index.html${V}
`),process.stderr.write(`Use 'loki proof list' to see available proofs.
`),1;if(X)return s8(Q,q,e(I$(),Q,"proof.json"));if((await F(["gh","--version"],{timeoutMs:5000})).exitCode!==0)return process.stderr.write(`${O}gh CLI not found${V}
...
`);let U=I1(Q);if(U){let M=p($$(U.cost).usd),k=p($$(U.files_changed).count),S=p($$(U.council).final_verdict),v=$$(U.redaction);process.stdout.write(`  - cost.usd:        ${M}
`),process.stdout.write(`  - files_changed:   ${k}
...
`),0}let H=m8(e(u8(),"loki-proof-")),B=e(H,"index.html");g8(q,B),process.stdout.write(`Uploading proof page...
`);let G=`Loki Mode proof-of-run ${Q}`,Y=["gh","gist","create",B,"--desc",G];if(z!=="")Y.push(z);let T=await F(Y,{timeoutMs:60000});try{f8(H,{recursive:!0,force:!0})}catch{}if(T.exitCode!==0)return process.stderr.write(`${O}Failed to create gist${V}
`),process.stderr.write(`${T.stdout}${T.stderr}
`),1;return process.stdout.write(`${N}Shared: ${T.stdout.trim()}${V}
`),0}async function r8($){let Q=$[0],Z=$.slice(1);if(Q===void 0||Q==="help"||Q==="--help"||Q==="-h")return process.stdout.write(l8),Q===void 0?1:0;switch(Q){case"list":return d8();case"show":return o8(Z[0]);case"open":return n8(Z[0]);case"share":return t8(Z);default:return process.stderr.write(`${O}Unknown subcommand: ${Q}${V}
`),process.stderr.write(`Run 'loki proof --help' for usage.
`),1}}var l8;var T6=L(()=>{C();d();c();B6();l8=`${R}loki proof${V} - inspect and share proof-of-run artifacts
...
`});var L6={};b(L6,{runCrash:()=>q7});import{existsSync as A6,readdirSync as i8,readFileSync as e8}from"fs";import{join as w6}from"path";function L$($){return $===void 0||$===null?"-":String($)}function f$($,Q){return $.length>=Q?$:$+" ".repeat(Q-$.length)}function _6(){return w6(P(),"crash")}function L1(){let $=_6();if(!A6($))return[];try{return i8($,{withFileTypes:!0}).filter((Q)=>Q.isFile()&&Q.name.endsWith(".json")).map((Q)=>Q.name.slice(0,-5)).sort()}catch{return[]}}function Q7($){if($.length===0)return!1;if($.includes("/")||$.includes("\\"))return!1;if($.includes(".."))return!1;return!0}function u$($){if(!Q7($))return null;let Q=w6(_6(),`${$}.json`);if(!A6(Q))return null;try{return JSON.parse(e8(Q,"utf8"))}catch{return{}}}function Z7(){let $=L1();if($.length===0)return process.stdout.write(`${_}No crash reports found.${V} Nothing has been captured in .loki/crash/.
...
`),0}function I6($){let Q=u$($);if(Q!==null)return{id:$,report:Q};for(let Z of L1()){let z=u$(Z);if(z&&String(z.fingerprint??"")===$)return{id:Z,report:z}}return null}function z7($){if(!$)return process.stderr.write(`${O}Missing crash id.${V} Use 'loki crash' to list reports.
`),2;let Q=I6($);if(Q===null)return process.stderr.write(`${O}Crash report not found: ${$}${V}
...
`),W=new URLSearchParams({title:X,body:K});return`${$7}?${W.toString()}`}function K7($){let Q;if($){if(Q=I6($),Q===null)return process.stderr.write(`${O}Crash report not found: ${$}${V}
`),process.stderr.write(`Use 'loki crash' to see available reports.
`),1}else{let Z=L1();if(Z.length===0)return process.stdout.write(`${_}No crash reports found.${V} Nothing to submit.
`),0;let z=Z[Z.length-1],X=u$(z)??{};Q={id:z,report:X}}return process.stdout.write(`${R}Scrubbed payload (this is the ENTIRE report):${V}
...
`),process.stdout.write(`${N}The payload above is exactly what the URL contains.${V}
`),process.stdout.write(`See docs/PRIVACY.md for what is and is not collected.
`),0}async function q7($){let Q=$[0];switch(Q){case void 0:case"list":return Z7();case"--help":case"-h":case"help":return process.stdout.write(O6),0;case"show":return z7($[1]);case"submit":return K7($[1]);default:return process.stderr.write(`${O}Unknown crash subcommand: ${Q}${V}
`),process.stdout.write(O6),2}}var $7="https://github.com/asklokesh/loki-mode/issues/new",O6;var P6=L(()=>{C();c();O6=`${R}loki crash${V} - inspect and manually submit local crash reports
...
`});var k6={};b(k6,{runWiki:()=>H7});import{existsSync as P1,readFileSync as j6}from"fs";import{join as j1,resolve as V7}from"path";function W7(){return j1(process.cwd(),".loki","wiki")}function U7($){let Q="";for(let X of $){if(X==="--help"||X==="-h")return process.stdout.write(`Usage: loki wiki show [section]
...
`),1;if(Q){if(!J7.has(Q))return process.stderr.write(`${O}No such section: ${Q} (try: architecture, modules, data-flow)${V}
`),1;let X=j1(Z,`${Q}.md`);if(!P1(X))return process.stderr.write(`${O}Section not generated: ${Q}${V}
`),1;return process.stdout.write(j6(X,"utf8")),0}let z=j1(Z,"index.md");if(!P1(z))return process.stderr.write(`${O}Wiki index not found. Run 'loki wiki generate'.${V}
`),1;return process.stdout.write(j6(z,"utf8")),0}async function R6($,Q){let Z=V7(g,"autonomy","loki"),z=3600000,X=Bun.spawn({cmd:[Z,"wiki",$,...Q],stdin:"inherit",stdout:"inherit",stderr:"inherit",env:{...process.env,LOKI_LEGACY_BASH:"1"}}),q=setTimeout(()=>{try{X.kill("SIGKILL")}catch{}},3600000);try{return await X.exited}finally{clearTimeout(q)}}async function H7($){let Q=$[0],Z=$.slice(1);switch(Q){case void 0:case"help":case"--help":case"-h":return process.stdout.write(F6),0;case"show":return U7(Z);case"generate":return R6("generate",Z);case"ask":return R6("ask",Z);default:return process.stderr.write(`${O}Unknown wiki command: ${Q}${V}
`),process.stdout.write(F6),1}}var F6,J7;var x6=L(()=>{C();c();F6=`${R}loki wiki${V} - Auto-generated, cited codebase wiki + Q&A
...
`,J7=new Set(["architecture","modules","data-flow"])});var R1={};b(R1,{renderFindingsForPrompt:()=>T7,loadPreviousFindings:()=>F1,findLatestReviewDir:()=>C6,_parseReviewerOutputForTests:()=>O7});import{existsSync as S6,readFileSync as E6,readdirSync as N6,statSync as G7}from"fs";import{join as c$}from"path";function M7($){let Q=$.toLowerCase();if(Q==="critical")return"Critical";if(Q==="high")return"High";if(Q==="medium")return"Medium";return"Low"}function D6($,Q,Z,z){let X=[],q=$.split(/\r?\n/);for(let K of q){let W=K.trim();if(W.length===0)continue;let J=W.replace(/^[-*]\s*/,""),U=B7.exec(J);if(!U||!U[1]||!U[2])continue;let H=M7(U[1]),B=U[2].trim(),G=Y7.exec(B),Y=G&&G[1]?G[1]:null,T=G&&G[2]?Number.parseInt(G[2],10):null;X.push({reviewId:Z,iteration:z,reviewer:Q,severity:H,description:B,file:Y,line:Number.isFinite(T)?T:null,raw:W})}return X}function C6($,Q){let Z=c$($,"quality","reviews");if(!S6(Z))return null;let z;try{z=N6(Z)}catch{return null}let X=Q===void 0?z.filter((W)=>W.startsWith("review-")):z.filter((W)=>W.endsWith(`-${Q}`)&&W.startsWith("review-"));if(X.length===0)return null;X.sort();let q=X[X.length-1];if(!q)return null;let K=c$(Z,q);try{if(!G7(K).isDirectory())return null}catch{return null}return K}function F1($,Q){let Z=C6($,Q);if(Z===null)return{reviewDir:null,reviewId:null,iteration:null,findings:[]};let z=null,X=null,q=c$(Z,"aggregate.json");if(S6(q))try{let U=E6(q,"utf-8"),H=JSON.parse(U);if(typeof H.review_id==="string")z=H.review_id;if(typeof H.iteration==="number")X=H.iteration}catch{}let K;try{K=N6(Z)}catch{return{reviewDir:Z,reviewId:z,iteration:X,findings:[]}}let W=new Set(["diff.txt","files.txt","anti-sycophancy.txt"]),J=[];for(let U of K){if(!U.endsWith(".txt"))continue;if(W.has(U))continue;if(U.endsWith("-prompt.txt"))continue;let H=U.replace(/\.txt$/,""),B;try{B=E6(c$(Z,U),"utf-8")}catch{continue}J.push(...D6(B,H,z??"",X??-1))}return{reviewDir:Z,reviewId:z,iteration:X,findings:J}}function T7($){if($.length===0)return"";let Q=["Critical","High","Medium","Low"],Z=new Map;for(let X of Q)Z.set(X,[]);for(let X of $){let q=Z.get(X.severity);if(q)q.push(X)}let z=[];z.push("PREVIOUS REVIEWER FINDINGS (must address each, or supply counter-evidence in .loki/state/counter-evidence-<iter>.json):");for(let X of Q){let q=Z.get(X)??[];if(q.length===0)continue;z.push(`  [${X}] (${q.length}):`);for(let K of q){let W=K.file?` (${K.file}${K.line!==null?":"+K.line:""})`:"";z.push(`    - ${K.description}${W} -- via ${K.reviewer}`)}}return z.join(`
`)}function O7($,Q,Z="review-test",z=0){return D6($,Q,Z,z)}var B7,Y7;var p$=L(()=>{B7=/\[(Critical|High|Medium|Low)\]\s*(.+)/i,Y7=/([\w.\-/]+\.[a-zA-Z]+):(\d+)/});import{existsSync as A7}from"fs";import{join as w7}from"path";async function b6($,Q){let Z=w7($,"memory");if(!A7(Z))return{stored:!1,reason:"memory dir not initialized"};let z=Math.max(0,Math.floor(Q.durationSeconds??0)),X={_LOKI_PROJECT_DIR:g,_LOKI_TARGET_DIR:process.cwd(),_LOKI_TASK_ID:Q.taskId,_LOKI_OUTCOME:Q.outcome,_LOKI_PHASE:Q.phase,_LOKI_GOAL:Q.goal,_LOKI_DURATION:String(z),_LOKI_LOKI_DIR:$},K=await z$(`
import os, sys
...
`),2;let Z=P();try{let X=(await Promise.resolve().then(() => (p$(),R1))).loadPreviousFindings(Z,Q);if(X.findings.length===0)return process.stdout.write(`reflect: no findings for iter ${Q} (nothing to do)
`),0;let q=j$(Z,"state");s7(q,{recursive:!0}),A$(j$(q,`findings-${Q}.json`),{review_id:X.reviewId,iteration:Q,findings:X.findings});let K=await Promise.resolve().then(() => (l$(),g6)),W=0,J=0;if(process.env.LOKI_AUTO_LEARNINGS!=="0"){let H=X.findings.filter((G)=>G.severity==="Critical"||G.severity==="High"),B=n6.fn??K.appendFromGateFailure;for(let G of H)try{await B(Z,Q,G,{episodeBridge:null}),W+=1}catch(Y){J+=1,process.stderr.write(`reflect: learning append failed for finding ${G.reviewer}/${G.severity}: ${Y.message}
...
`),0;let K=(await Promise.resolve().then(() => (p$(),R1))).loadPreviousFindings(Z,Q),W=K.findings.filter((T)=>T.severity==="Critical"||T.severity==="High");if(W.length===0)return process.stdout.write(`override: no blocking findings for iter ${Q} (skip)
`),0;let J=async(T)=>{return{judge:T.judge,verdict:"REJECT_OVERRIDE",reasoning:`[stub] proofType=${T.evidence.proofType}: self-supplied counter-evidence cannot lift a trust-gate BLOCK on the agent-authored route. Fix the finding, or use the human-escape path (rm .loki/PAUSE; optionally .loki/HUMAN_INPUT.md). The only adjudicated override is the Bun-route real-LLM judge.`}},U=await z.runOverrideCouncil(W,X,J);await z.recordOverrideOutcome(Z,Q,U,W);let H=j$(Z,"quality","reviews");if(a7(H))try{let T=t7(H).filter((k)=>k.startsWith("review-")).sort(),M=T[T.length-1];if(M&&r7(j$(H,M)).isDirectory())A$(j$(H,M,`override-${Q}.json`),{review_id:K.reviewId,iteration:Q,approved_finding_ids:Array.from(U.approvedFindingIds),rejected_finding_ids:Array.from(U.rejectedFindingIds),votes:U.votes})}catch{}let B=U.approvedFindingIds.size,G=U.rejectedFindingIds.size;if(G===0&&B>0)process.stdout.write(`override: LIFTED -- ${B} approved, ${G} rejected
...
`,zZ;var s6=L(()=>{C();m$();n6={fn:null};zZ=x1});r$();function C1(){return process.stdout.write(`Loki Mode v${R$()}
`),0}d();c();C();import{readFileSync as HQ,existsSync as GQ}from"fs";import{resolve as BQ}from"path";var YQ=["claude","codex","cline","aider"];function h1(){let $=BQ(P(),"state","provider");if(!GQ($))return"";try{return HQ($,"utf-8").trim()}catch{return""}}function MQ($,Q){return $||Q||process.env.LOKI_PROVIDER||"claude"}function TQ($){let Q=h1(),Z=MQ($,Q);switch(process.stdout.write(`${R}Current Provider${V}
`),process.stdout.write(`
`),process.stdout.write(`${I}Provider:${V} ${Z}
`),Z){case"claude":process.stdout.write(`${N}Status:${V}   Full features (subagents, parallel, MCP)
`);break;case"cline":process.stdout.write(`${N}Status:${V}   Near-full mode (subagents, MCP, 12+ providers)
...
`),0}async function OQ(){let Q=h1()||process.env.LOKI_PROVIDER||"claude";process.stdout.write(`${R}Available Providers${V}
`),process.stdout.write(`
`);let Z=await Promise.all(YQ.map(async(q)=>[q,await f(q)!==null])),z=new Map;for(let[q,K]of Z)z.set(q,K?`${N}installed${V}`:`${O}not installed${V}`);let X=[["claude","claude  - Claude Code (Anthropic)    "],["codex","codex   - Codex CLI (OpenAI)         "],["cline","cline   - Cline (multi-provider)     "],["aider","aider   - Aider (terminal pair prog) "]];for(let[q,K]of X){let W=Q===q?` ${I}(current)${V}`:"";process.stdout.write(`  ${K} ${z.get(q)}${W}
`)}return process.stdout.write(`
`),process.stdout.write(`Set provider: ${I}loki provider set <name>${V}
`),0}function AQ(){return process.stdout.write(`${R}Loki Mode Provider Management${V}
`),process.stdout.write(`
...
try:
    ...
    print(f'Error: {e}')
`.trim(),X=await z$(z,{cwd:g});return process.stdout.write(X.stdout),0}let Q=J$(P(),"memory","index.json");if(!v1(Q))return process.stdout.write(`No index found
...
`),0;return process.stdout.write(Z.stdout),0}async function m1($){switch($[0]??"list"){case"list":case"ls":return PQ();case"index":return jQ($[1]==="rebuild");default:{let Z=J$(g,"autonomy","loki"),z=3600000,X=Bun.spawn({cmd:[Z,"memory",...$],stdin:"inherit",stdout:"inherit",stderr:"inherit",env:{...process.env,LOKI_LEGACY_BASH:"1"}}),q=setTimeout(()=>{try{X.kill("SIGKILL")}catch{}},3600000);try{return await X.exited}finally{clearTimeout(q)}}}}C();V$();d();import{resolve as FQ,join as RQ}from"path";import{existsSync as $1,readFileSync as kQ}from"fs";import{homedir as xQ}from"os";import{spawnSync as c1}from"child_process";var p1=3000;function EQ(){let $=(process.env.LOKI_TELEMETRY??"").toLowerCase();if($==="off")return!1;if(process.env.LOKI_TELEMETRY_DISABLED==="true")return!1;if(process.env.DO_NOT_TRACK==="1")return!1;let Q=!1,Z=!1;try{let z=RQ(xQ(),".loki","config");if($1(z)){let X=kQ(z,"utf8");for(let q of X.split(`
`)){let K=q.replace(/\r$/,"");if(K==="TELEMETRY_DISABLED=true")Q=!0;if(K==="TELEMETRY_ENABLED=true")Z=!0}}}catch{}if(Q)return!1;if($==="on"||Z)return!0;return!1}var k$=!1;function SQ(){return FQ(g,"autonomy","lib","crash_capture.py")}function NQ($,Q){let Z=[$,"--error-class",Q.errorClass,"--message",Q.message];if(Q.stack!==void 0)Z.push("--stack",Q.stack);if(Q.rarvPhase!==void 0)Z.push("--rarv-phase",Q.rarvPhase);if(Q.exitCode!==void 0)Z.push("--exit-code",String(Q.exitCode));if(Q.frictionKind!==void 0)Z.push("--friction-kind",Q.frictionKind);return Z.push("--target-dir",Q.targetDir??process.cwd()),Z}function DQ(){if($1("/opt/homebrew/bin/python3.12"))return"/opt/homebrew/bin/python3.12";for(let Q of["python3.12","python3"])try{let Z=c1("sh",["-c",`command -v ${Q}`],{timeout:p1,encoding:"utf8"});if(Z.status===0){let z=(Z.stdout||"").trim();if(z)return z}}catch{}return null}function g1($){try{if(!EQ())return;let Q=SQ();if(!$1(Q))return;let Z=DQ();if(!Z)return;let z=NQ(Q,$);c1(Z,z,{timeout:p1,stdio:"ignore"})}catch{}}function f1($,Q){if($ instanceof Error){let z={errorClass:$.name&&$.name.length>0?$.name:Q,message:$.message};if($.stack)z.stack=$.stack;return z}return{errorClass:Q,message:String($)}}var u1=!1;function l1(){if(u1)return;u1=!0,process.on("uncaughtException",($)=>{if(!k$){k$=!0;let Q=f1($,"UncaughtException");g1({errorClass:Q.errorClass,message:Q.message,...Q.stack!==void 0?{stack:Q.stack}:{},exitCode:1})}try{process.stderr.write(`${$&&$.stack||String($)}
...
`)}catch{}process.exit(1)})}var t6=`Loki Mode (TypeScript port, Phase 2 of bash->Bun migration)
