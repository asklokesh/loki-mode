// @bun
var t6=Object.defineProperty;var r6=($)=>$;function i6($,Q){this[$]=r6.bind(null,Q)}var b=($,Q)=>{for(var Z in Q)t6($,Z,{get:Q[Z],enumerable:!0,configurable:!0,set:i6.bind(Q,Z)})};var P=($,Q)=>()=>($&&(Q=$($=0)),Q);var q$=import.meta.require;var D1={};b(D1,{lokiDir:()=>j,homeLokiDir:()=>s$,findRepoRootForVersion:()=>a$,REPO_ROOT:()=>g});import{resolve as a,dirname as n$}from"path";import{fileURLToPath as e6}from"url";import{existsSync as F$}from"fs";import{homedir as $Q}from"os";function QQ(){let $=S1;for(let Q=0;Q<6;Q++){if(F$(a($,"VERSION"))&&F$(a($,"autonomy/run.sh")))return $;let Z=n$($);if(Z===$)break;$=Z}return a(S1,"..","..","..")}function a$($){let Q=$;for(let Z=0;Z<6;Z++){if(F$(a(Q,"VERSION"))&&F$(a(Q,"autonomy/run.sh")))return Q;let z=n$(Q);if(z===Q)break;Q=z}return a($,"..","..","..")}function j(){return process.env.LOKI_DIR??a(process.cwd(),".loki")}function s$(){return a($Q(),".loki")}var S1,g;var C=P(()=>{S1=n$(e6(import.meta.url));g=QQ()});import{readFileSync as ZQ}from"fs";import{resolve as zQ,dirname as XQ}from"path";import{fileURLToPath as KQ}from"url";function R$(){if(Q$!==null)return Q$;let $="7.67.0";if(typeof $==="string"&&$.length>0)return Q$=$,Q$;try{let Q=XQ(KQ(import.meta.url)),Z=a$(Q);Q$=ZQ(zQ(Z,"VERSION"),"utf-8").trim()}catch{Q$="unknown"}return Q$}var Q$=null;var t$=P(()=>{C()});var b1={};b(b1,{runOrThrow:()=>qQ,run:()=>R,commandVersion:()=>JQ,commandExists:()=>f,ShellError:()=>r$});async function R($,Q={}){let Z=Bun.spawn({cmd:[...$],stdout:"pipe",stderr:"pipe",env:Q.env?{...process.env,...Q.env}:process.env,cwd:Q.cwd}),z,X;if(Q.timeoutMs&&Q.timeoutMs>0)z=setTimeout(()=>{try{Z.kill("SIGTERM")}catch{}X=setTimeout(()=>{try{Z.kill("SIGKILL")}catch{}},2000)},Q.timeoutMs);try{let[q,K,W]=await Promise.all([new Response(Z.stdout).text(),new Response(Z.stderr).text(),Z.exited]);return{stdout:q,stderr:K,exitCode:W}}finally{if(z)clearTimeout(z);if(X)clearTimeout(X)}}async function qQ($,Q={}){let Z=await R($,Q);if(Z.exitCode!==0)throw new r$(`command failed (${Z.exitCode}): ${$.join(" ")}`,Z.exitCode,Z.stdout,Z.stderr);return Z}async function f($){let Q=VQ($),Z=await R(["sh","-c",`command -v ${Q}`],{timeoutMs:5000});if(Z.exitCode===0)return Z.stdout.trim()||null;return null}function VQ($){if(!/^[A-Za-z0-9._/-]+$/.test($))throw Error(`refused to shell-escape suspect token: ${$}`);return $}async function JQ($,Q="--version"){if(!await f($))return null;let z=await R([$,Q],{timeoutMs:5000});if(z.exitCode!==0)return null;return((z.stdout||z.stderr).split(/\r?\n/)[0]?.trim()??"")||null}var r$;var d=P(()=>{r$=class r$ extends Error{message;exitCode;stdout;stderr;constructor($,Q,Z,z){super($);this.message=$;this.exitCode=Q;this.stdout=Z;this.stderr=z;this.name="ShellError"}}});function s($){return WQ?"":$}var WQ,T,S,_,wZ,I,k,h,V;var c=P(()=>{WQ=(process.env.NO_COLOR??"").length>0;T=s("\x1B[0;31m"),S=s("\x1B[0;32m"),_=s("\x1B[1;33m"),wZ=s("\x1B[0;34m"),I=s("\x1B[0;36m"),k=s("\x1B[1m"),h=s("\x1B[2m"),V=s("\x1B[0m")});import{existsSync as wQ}from"fs";async function Z$(){if(Y$!==void 0)return Y$;let $="/opt/homebrew/bin/python3.12";if(wQ($))return Y$=$,$;let Q=await f("python3.12");if(Q)return Y$=Q,Q;let Z=await f("python3");return Y$=Z,Z}async function z$($,Q={}){let Z=await Z$();if(!Z)return{stdout:"",stderr:"python3 not found",exitCode:127};return R([Z,"-c",$],Q)}var Y$;var V$=P(()=>{d()});var e1={};b(e1,{runStatus:()=>uQ});import{existsSync as y,readFileSync as W$,readdirSync as d1,statSync as o1}from"fs";import{resolve as D,basename as DQ}from"path";import{homedir as CQ}from"os";function n1($){let Q=Math.trunc($);if(Q>=1e6)return`${(Math.trunc(Q/1e6*10)/10).toFixed(1)}M`;if(Q>=1000)return`${(Math.trunc(Q/1000*10)/10).toFixed(1)}K`;return String(Q)}function a1($,Q,Z){if(Q===0)return null;let z=Math.trunc($*100/Q),X=Math.trunc($*x$/Q);if(X>x$)X=x$;let q=x$-X,K=S;if(z>=80)K=T;else if(z>=50)K=_;let W="=".repeat(Math.max(0,X))+" ".repeat(Math.max(0,q)),J=n1($),U=n1(Q);return`  ${k}${Z}${V} ${K}[${W}]${V} ${z}% (${J} / ${U})`}async function hQ(){if(await f("jq"))return!0;return process.stdout.write(`${T}Error: jq is required but not installed.${V}
`),process.stdout.write(`Install with:
`),process.stdout.write(`  brew install jq    (macOS)
`),process.stdout.write(`  apt install jq     (Debian/Ubuntu)
`),process.stdout.write(`  yum install jq     (RHEL/CentOS)
`),!1}function E$($){if(!Number.isFinite($)||$<=0)return!1;try{return process.kill($,0),!0}catch{return!1}}function N$($){if(!y($))return null;try{let Q=W$($,"utf-8").trim();if(!Q)return null;let Z=Number.parseInt(Q,10);return Number.isFinite(Z)?Z:null}catch{return null}}function yQ($){let Q=[],Z=N$(D($,"loki.pid"));if(Z!==null&&E$(Z))Q.push(`global:${Z}`);let z=D($,"sessions");if(y(z)){let X=[];try{X=d1(z)}catch{X=[]}for(let q of X){let K=D(z,q);try{if(!o1(K).isDirectory())continue}catch{continue}let W=D(K,"loki.pid"),J=N$(W);if(J!==null&&E$(J))Q.push(`${q}:${J}`)}}if(y($)){let X=[];try{X=d1($)}catch{X=[]}for(let q of X){if(!q.startsWith("run-")||!q.endsWith(".pid"))continue;let K=D($,q);try{if(!o1(K).isFile())continue}catch{continue}let W=DQ(q,".pid").slice(4),J=N$(K);if(J!==null&&E$(J)){if(!Q.some((H)=>H.startsWith(`${W}:`)))Q.push(`${W}:${J}`)}}}return Q}async function s1($,Q){let Z=await R(["jq","-r",$,Q]);if(Z.exitCode!==0)return null;return Z.stdout.trim()}function t1($,Q){try{let Z=W$($,"utf-8"),X=JSON.parse(Z)[Q];if(typeof X==="number"){if(Q==="budget_used"){let q=Math.round(X*100)/100;if(Number.isInteger(q))return String(q);return String(q)}return String(X)}if(X===void 0||X===null)return"0";return String(X)}catch{return"0"}}function r1($,Q,Z){try{let z=W$($,"utf-8"),q=JSON.parse(z)[Q];if(typeof q==="number"&&Number.isFinite(q))return q;return Z}catch{return Z}}async function vQ(){let $=j();if(!await hQ())return 1;if(!y($))return process.stdout.write(`${k}Loki Mode Status${V}
...
`)}let U=D($,"metrics","budget.json");if(y(U)){let G=t1(U,"budget_limit"),O=t1(U,"budget_used");if(G!=="0"){process.stdout.write(`${I}Budget:${V} $${O} / $${G}
...
`)}let H=D($,"state","context-usage.json");if(y(H)){let G=r1(H,"window_size",200000),O=r1(H,"used_tokens",0),M=a1(O,G,"Context");if(M!==null)process.stdout.write(`${M}
`)}let B=[D($,"dashboard","dashboard.pid"),D(CQ(),".loki","dashboard","dashboard.pid")].find((G)=>y(G))??"";if(B&&y(B)){let G=N$(B);if(G!==null&&E$(G)){let O=D(B,".."),M=(u,w)=>{let E=D(O,u);try{return y(E)?W$(E,"utf-8").trim()||w:w}catch{return w}},L=M("scheme","http"),x=M("host","127.0.0.1"),v=M("port",process.env.LOKI_DASHBOARD_PORT||"57374");if(x==="0.0.0.0")x="127.0.0.1";process.stdout.write(`${I}Dashboard:${V} ${L}://${x}:${v}/
...
`),1;let Q=g,Z=j(),z=process.env.LOKI_DASHBOARD_PORT||"57374",X=process.env.LOKI_PROVIDER||"claude",q=await R([$,"-c",bQ,Q,Z,z,X],{timeoutMs:30000});if(q.exitCode!==0)return process.stderr.write(`{"error": "Failed to generate JSON status. Ensure python3 is available."}
...
print(json.dumps(result, indent=2))
`;var $0=P(()=>{d();V$();c();C()});var Z0={};b(Z0,{emitDeprecatedAlias:()=>$1,deprecatedAliasShouldSuppress:()=>Q0});function Q0($){let Q=$[0];if(Q!==void 0&&pQ.has(Q))return!0;for(let Z of $)if(cQ.has(Z))return!0;return!1}function $1($,Q,Z){if(Q0(Z))return;process.stderr.write(`note: 'loki ${$}' is now 'loki ${Q}'. The old form still works.
`)}var cQ,pQ;var Q1=P(()=>{cQ=new Set(["--json","-q","--quiet"]),pQ=new Set(["json","csv","timeline"])});var q0={};b(q0,{runStats:()=>sQ,computeStats:()=>K0});import{readdirSync as z0,readFileSync as lQ,statSync as X0}from"fs";import{join as t}from"path";function U$($){try{if(!X0($).isFile())return null;return JSON.parse(lQ($,"utf-8"))}catch{return null}}function X1($){try{return X0($).isDirectory()}catch{return!1}}function dQ($){if(!X1($))return[];try{let Q=z0($).filter((Z)=>Z.startsWith("iteration-")&&Z.endsWith(".json"));return Q.sort(),Q.map((Z)=>t($,Z))}catch{return[]}}function H$($){return Math.trunc($).toLocaleString("en-US")}function Z1($){let Q=Math.trunc($);if(Q<60)return`${Q}s`;let Z=Math.trunc(Q/3600),z=Math.trunc(Q%3600/60),X=Q%60;if(Z>0)return`${Z}h ${String(z).padStart(2,"0")}m`;return`${z}m ${String(X).padStart(2,"0")}s`}function r($,Q=0){let Z=Math.pow(10,Q);return Math.round($*Z)/Z}function B$($,Q){return $.toFixed(Q)}function z1($,Q){return $.length>=Q?$:$+" ".repeat(Q-$.length)}function oQ($){let Q="N/A",Z=0,z=U$(t($,"state","orchestrator.json"));if(z&&typeof z==="object"){if(typeof z.currentPhase==="string")Q=z.currentPhase;if(typeof z.currentIteration==="number")Z=z.currentIteration}let X=t($,"metrics","efficiency"),q=dQ(X),K=[];for(let F of q){let N=U$(F);if(N&&typeof N==="object")K.push(N)}if(K.length>0)Z=Math.max(Z,K.length);let W=K.reduce((F,N)=>F+(N.input_tokens??0),0),J=K.reduce((F,N)=>F+(N.output_tokens??0),0),U=W+J,H=K.reduce((F,N)=>F+(N.cost_usd??0),0),Y=K.reduce((F,N)=>F+(N.duration_seconds??0),0),B=0,G=0,O=U$(t($,"metrics","budget.json"));if(O&&typeof O==="object"){if(typeof O.budget_limit==="number")B=O.budget_limit;if(typeof O.budget_used==="number")G=O.budget_used}let M=0,L=0,x=U$(t($,"state","quality-gates.json"));if(x&&typeof x==="object"){if(Array.isArray(x)){for(let F of x)if(L+=1,F===!0)M+=1;else if(F&&typeof F==="object"){let N=F;if(N.passed===!0||N.status==="passed")M+=1}}else for(let F of Object.values(x))if(typeof F==="boolean"){if(L+=1,F)M+=1}else if(F&&typeof F==="object"){L+=1;let N=F;if(N.passed===!0||N.status==="passed")M+=1}}let v={},u=U$(t($,"quality","gate-failure-count.json"));if(u&&typeof u==="object"&&!Array.isArray(u)){let F={};for(let[N,l]of Object.entries(u))if(typeof l==="number")F[N]=l;v=F}let w=0,E=0,n=0,o$=t($,"quality");if(X1(o$)){let F=[];try{F=z0(o$)}catch{F=[]}for(let N of F){if(!N.endsWith(".json")||N==="gate-failure-count.json")continue;let l=U$(t(o$,N));if(!l||typeof l!=="object")continue;if(!(("verdict"in l)||("approved"in l)||("reviewers"in l)))continue;w+=1;let N1=(l.verdict??"").toString().toLowerCase();if(l.approved===!0||["approved","approve","pass"].includes(N1))E+=1;else if(["revision","revise","changes_requested","reject"].includes(N1))n+=1}}return{phase:Q,iterationCount:Z,iterations:K,totalInput:W,totalOutput:J,totalTokens:U,totalCost:H,totalDuration:Y,budgetLimit:B,budgetUsed:G,gatesPassed:M,gatesTotal:L,gateFailures:v,reviewsTotal:w,reviewsApproved:E,reviewsRevision:n}}function nQ($,Q){let Z=$.iterationCount,z={session:{iterations:Z,duration_seconds:$.totalDuration,phase:$.phase},tokens:{input:$.totalInput,output:$.totalOutput,total:$.totalTokens,cost_usd:r($.totalCost,2)},quality:{gates_passed:$.gatesPassed,gates_total:$.gatesTotal,reviews_total:$.reviewsTotal,reviews_approved:$.reviewsApproved,reviews_revision:$.reviewsRevision,gate_failures:$.gateFailures},efficiency:{avg_tokens_per_iteration:Z>0?r($.totalTokens/Z,0):0,avg_cost_per_iteration:Z>0?r($.totalCost/Z,2):0,avg_duration_per_iteration:Z>0?r($.totalDuration/Z,1):0},budget:{used:r($.budgetUsed,2),limit:$.budgetLimit,percent:$.budgetLimit>0?r($.budgetUsed/$.budgetLimit*100,1):0}};if(Q)z.iterations=$.iterations.map((K,W)=>({number:W+1,input_tokens:K.input_tokens??0,output_tokens:K.output_tokens??0,cost_usd:r(K.cost_usd??0,2),duration_seconds:K.duration_seconds??0}));let X=JSON.stringify(z,null,2);function q(K,W){if(!W)return;let J=new RegExp(`("${K}": )(-?\\d+)(,?)$`,"m");X=X.replace(J,(U,H,Y,B)=>`${H}${Y}.0${B}`)}if(q("avg_duration_per_iteration",Z>0&&Number.isInteger(z.efficiency.avg_duration_per_iteration)),q("percent",$.budgetLimit>0&&Number.isInteger(z.budget.percent)),q("cost_usd",Z>0&&Number.isInteger(z.tokens.cost_usd)),Q)X=X.replace(/("cost_usd": )(-?\d+)(,?)$/gm,(K,W,J,U)=>`${W}${J}.0${U}`);return X}function aQ($,Q){let Z=[];if(Z.push("Loki Mode Session Statistics"),Z.push("============================"),Z.push(""),Z.push("Session"),Z.push(`  Iterations completed: ${$.iterationCount}`),Z.push(`  Duration: ${Z1($.totalDuration)}`),Z.push(`  Current phase: ${$.phase}`),Z.push(""),Z.push("Token Usage"),$.iterations.length>0)Z.push(`  Input tokens:  ${H$($.totalInput)}`),Z.push(`  Output tokens: ${H$($.totalOutput)}`),Z.push(`  Total tokens:  ${H$($.totalTokens)}`),Z.push(`  Estimated cost: $${B$($.totalCost,2)}`);else Z.push("  N/A (no iteration metrics found)");if(Z.push(""),Z.push("Quality Gates"),$.gatesTotal>0){let z=Math.round($.gatesPassed/$.gatesTotal*100);Z.push(`  Gates passed: ${$.gatesPassed}/${$.gatesTotal} (${z}%)`)}else Z.push("  Gates passed: N/A");if($.reviewsTotal>0){let z=[];if($.reviewsApproved>0)z.push(`${$.reviewsApproved} approved`);if($.reviewsRevision>0)z.push(`${$.reviewsRevision} revision requested`);let X=z.length>0?z.join(", "):"N/A";Z.push(`  Code reviews: ${$.reviewsTotal} (${X})`)}if(Object.keys($.gateFailures).length>0){let z=Object.entries($.gateFailures).filter(([,X])=>X>0).map(([X,q])=>`${X} (${q})`);if(z.length>0)Z.push(`  Gate failures: ${z.join(", ")}`)}if(Z.push(""),Z.push("Efficiency"),$.iterationCount>0&&$.iterations.length>0){let z=Math.round($.totalTokens/$.iterationCount),X=$.totalCost/$.iterationCount,q=$.totalDuration/$.iterationCount;Z.push(`  Avg tokens/iteration: ${H$(z)}`),Z.push(`  Avg cost/iteration: $${B$(X,2)}`),Z.push(`  Avg duration/iteration: ${Z1(q)}`)}else Z.push("  N/A (no iteration metrics found)");if(Z.push(""),Z.push("Budget"),$.budgetLimit>0){let z=r($.budgetUsed/$.budgetLimit*100,1),X=Number.isInteger(z)?`${z}.0`:`${z}`;Z.push(`  Used: $${B$($.budgetUsed,2)} / $${B$($.budgetLimit,2)} (${X}%)`)}else if($.budgetUsed>0)Z.push(`  Used: $${B$($.budgetUsed,2)} (no limit set)`);else Z.push("  N/A");if(Q&&$.iterations.length>0)Z.push(""),Z.push("Per-Iteration Breakdown"),$.iterations.forEach((z,X)=>{let q=X+1,K=z1(H$(z.input_tokens??0),10),W=z1(H$(z.output_tokens??0),10),J=z.cost_usd??0,U=Z1(z.duration_seconds??0),H=z1(`${q}`,3);Z.push(`  #${H} input: ${K} output: ${W} cost: $${B$(J,2)}  time: ${U}`)});return Z.join(`
`)}function K0($){let Q=!1,Z=!1;for(let K of $)if(K==="--json")Q=!0;else if(K==="--efficiency")Z=!0;let z=j();if(!X1(z)){if(Q)return{exitCode:0,stdout:'{"error": "No active session"}'};return{exitCode:0,stdout:`${_}No active session found.${V}
Start a session with: loki start <prd>`}}let X=oQ(z);return{exitCode:0,stdout:Q?nQ(X,Z):aQ(X,Z)}}async function sQ($){let{emitDeprecatedAlias:Q}=await Promise.resolve().then(() => (Q1(),Z0));Q("stats","report session",$);let Z=K0($);return console.log(Z.stdout),Z.exitCode}var V0=P(()=>{C();c()});var T0={};b(T0,{runDoctor:()=>W3,pythonImportOk:()=>J1,httpReachable:()=>q1,checkTool:()=>B0,checkSkills:()=>G0,checkDisk:()=>V1,buildDoctorJson:()=>M0,_setPythonImportOkForTest:()=>Z3});import{existsSync as J0,lstatSync as tQ,readlinkSync as rQ,statfsSync as iQ}from"fs";import{spawnSync as eQ}from"child_process";import{homedir as W0}from"os";import{resolve as K1}from"path";function Q3($){let Q=$.match($3);return Q?Q[1]:null}async function U0($){try{let Q=await R([$,"--version"],{timeoutMs:5000}),Z=(Q.stdout||Q.stderr||"").trim();return Q3(Z)}catch{return null}}function H0($,Q){let Z=$.split(".").map((X)=>parseInt(X,10)),z=Q.split(".").map((X)=>parseInt(X,10));while(Z.length<2)Z.push(0);while(z.length<2)z.push(0);for(let X=0;X<2;X++){let q=Z[X]??0,K=z[X]??0;if(Number.isNaN(q)||Number.isNaN(K))return 0;if(q!==K)return q-K}return 0}async function B0($,Q,Z,z=null){let X=await f(Q),q=X!==null,K=q?await U0(Q):null,W="pass";if(!q)W=Z==="required"?"fail":"warn";else if(z&&K){if(H0(K,z)<0)W=Z==="required"?"fail":"warn"}return{name:$,command:Q,found:q,version:K,required:Z,min_version:z,status:W,path:X}}function V1(){let $=null;try{let Z=iQ(W0()),z=Number(Z.bavail)*Number(Z.bsize);$=Math.round(z/1073741824*10)/10}catch{$=null}let Q="pass";if($!==null){if($<1)Q="fail";else if($<5)Q="warn"}return{available_gb:$,status:Q}}async function q1($,Q=2000){try{return(await fetch($,{signal:AbortSignal.timeout(Q)})).ok}catch{return!1}}async function J1($,Q=!1){let Z=`import ${$}`,z=Q?30000:5000;if(!Q)return(await z$(Z,{timeoutMs:z})).exitCode===0;let X=await Z$();if(!X)return!1;return(await R([X,"-c",Z],{timeoutMs:z})).exitCode===0}function Z3($){C$.fn=$??J1}function G0(){let $=W0();return z3.map(({name:Q,dir:Z})=>{let z=K1($,Z),X=z,q=K1(z,"SKILL.md");if(J0(q))return{name:Q,path:X,status:"pass",detail:""};try{if(tQ(z).isSymbolicLink()){let W="unknown";try{W=rQ(z)}catch{}return{name:Q,path:X,status:"fail",detail:`(broken symlink -> ${W})`}}}catch{}return{name:Q,path:X,status:"warn",detail:"(not found - run 'loki setup-skill')"}})}async function Y0(){return Promise.all(X3.map(async($)=>{return{...await B0($.jsonName,$.cmd,$.required,$.min??null),displayName:$.displayName}}))}async function K3(){let Q=await f("sentrux")!==null,Z=Q?await U0("sentrux"):null;return{found:Q,version:Z,status:Q?"pass":"warn",required:"optional"}}async function q3(){let{openSync:$,statSync:Q,readSync:Z,closeSync:z,existsSync:X}=await import("fs"),{join:q}=await import("path"),K=65536,W=process.env.LOKI_DIR??".loki",J=q(W,"memory",".errors.log"),U=[],H=!1;try{if(X(J)){H=!0;let Y=Q(J).size,B=Math.max(0,Y-65536),G=Y-B,O=Buffer.alloc(G),M=$(J,"r");try{Z(M,O,0,G,B)}finally{z(M)}let x=O.toString("utf-8").split(`
`);if(B>0&&x.length>0)x=x.slice(1);x=x.map((v)=>v.trim()).filter((v)=>v.length>0),U=x.slice(-5)}}catch{U=[]}return{errors_log_path:H?J:null,recent_errors:U,recent_error_count:U.length,status:U.length===0?"pass":"warn"}}async function M0(){let Q=(await Y0()).map(({displayName:J,...U})=>U),Z=V1(),z=await K3(),X=await q3(),q=0,K=0,W=0;for(let J of Q)if(J.status==="pass")q++;else if(J.status==="fail")K++;else W++;if(Z.status==="pass")q++;else if(Z.status==="fail")K++;else W++;return{loki_mode_version:R$(),checks:Q,disk:Z,sentrux:z,memory:X,summary:{passed:q,failed:K,warnings:W,ok:K===0}}}function A($){switch($){case"pass":return`${S}PASS${V}`;case"fail":return`${T}FAIL${V}`;case"warn":return`${_}WARN${V}`}}function S$($){let Q=$.version?` (v${$.version})`:"",Z=$.displayName;if(!$.found){let z=$.required==="required"?"not found":$.required==="recommended"?"not found (recommended)":"not found (optional)";return`  ${A($.status)}  ${Z} - ${z}`}if($.min_version&&$.version&&H0($.version,$.min_version)<0){let z=$.required==="required"?"requires":"recommended";return`  ${A($.status)}  ${Z}${Q} - ${z} >= ${$.min_version}`}return`  ${A($.status)}  ${Z}${Q}`}function D$($,Q){if(Q==="pass")$.pass++;else if(Q==="fail")$.fail++;else $.warn++}function V3(){process.stdout.write(`${k}loki doctor${V} - Check system prerequisites
...
`)}async function J3(){process.stdout.write(`${k}Loki Mode Doctor${V}
...
`);for(let w of["node","python3","jq","git","curl"]){let E=Z.get(w);process.stdout.write(S$(E)+`
`),D$($,E.status)}process.stdout.write(`
`),process.stdout.write(`${I}AI Providers:${V}
`);let z=["claude","codex","cline","aider"],X={claude:"npm install -g @anthropic-ai/claude-code",codex:"npm install -g @openai/codex",cline:"npm install -g cline",aider:"pip install aider-chat"},q=!1;for(let w of z){let E=Z.get(w);if(process.stdout.write(S$(E)+`
`),!E.found&&X[w])process.stderr.write(`         ${_}Install: ${X[w]}${V}
`);if(D$($,E.status),E.found)q=!0}if(!q){if(process.stdout.write(`  ${A("fail")}  No AI provider CLI installed -- at least one is required
`),process.stdout.write(`         ${_}Install: npm install -g @anthropic-ai/claude-code${V}
`),$.fail++,process.stdout.isTTY){let w=K1(g,"autonomy/provider-offer.sh");if(J0(w))eQ("bash",[w,"report"],{stdio:"inherit"})}}process.stdout.write(`
...
`);let[U,H,Y]=await Promise.all([C$.fn("mcp"),C$.fn("numpy",!0),C$.fn("sentence_transformers",!0)]);if(U)process.stdout.write(`  ${A("pass")}  MCP SDK (Python)
...
`),$.warn++;if(await q1("http://localhost:8100/api/v2/heartbeat"))process.stdout.write(`  ${A("pass")}  ChromaDB server (port 8100)
...
`),$.warn++}let B=process.env.LOKI_MIROFISH_URL;if(B)if(await q1(`${B}/health`))process.stdout.write(`  ${A("pass")}  MiroFish server (${B})
...
`),$.warn++;if(await f("sentrux")){let w="unknown";try{let n=(await R(["sentrux","--version"],{timeoutMs:2000})).stdout.split(/\s+/).filter(Boolean).pop();if(n)w=n.replace(/^v/,"")}catch{}process.stdout.write(`  ${A("pass")}  sentrux ${w} (architectural drift gate: loki sentrux help)
...
`);let G=Z.get("bash");process.stdout.write(S$(G)+`
`),D$($,G.status);let O=Z.get("bun");if(O)process.stdout.write(S$(O)+`
`),D$($,O.status);let M=V1(),L=M.available_gb===null?null:Math.floor(M.available_gb);if(L===null)process.stdout.write(`  ${A("warn")}  Disk space: unable to determine
...
`);let u=await Z$();if(u!==null){let E=(await R([u,"-c","import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}')"],{timeoutMs:5000})).stdout.trim();if(E.startsWith("3.12"))process.stdout.write(`  ${A("pass")}  Python 3.12 (chromadb / sentence-transformers): ${E} at ${u}
...
`),process.stdout.write(`${k}Summary:${V} ${S}${$.pass} passed${V}, ${T}${$.fail} failed${V}, ${_}${$.warn} warnings${V}
...
`;var H6=P(()=>{V6();c()});function h8(){return process.env.LOKI_TIER||"oss"}function B6($){let Q=h8();if(Q==="oss")return{allowed:!0,notes:[]};if(!process.env.LOKI_LICENSE_KEY)return{allowed:!1,notes:[`${_}LOKI_TIER='${Q}' requested but no LOKI_LICENSE_KEY set.${V}`,`Hosted/enterprise license verification is not available yet (capability: ${$}).`,"OSS users: leave LOKI_TIER unset (or 'oss') -- everything stays free."]};return{allowed:!0,notes:[`${_}LOKI_LICENSE_KEY set but the verification backend is not available yet (R9 seam).${V}`]}}var G6=P(()=>{c()});var M6={};b(M6,{runProof:()=>t8});import{existsSync as _$,readdirSync as y8,readFileSync as Y6,mkdtempSync as v8,copyFileSync as m8,rmSync as g8}from"fs";import{join as e}from"path";import{tmpdir as f8}from"os";import{createInterface as u8}from"readline";import{readFile as c8}from"fs/promises";function $$($){return $&&typeof $==="object"?$:{}}function p($){return $===void 0||$===null?"-":String($)}function I$(){return e(j(),"proofs")}function _1($){let Q=e(I$(),$,"proof.json");if(!_$(Q))return null;try{return JSON.parse(Y6(Q,"utf8"))}catch{return{}}}function i($,Q){return $.length>=Q?$:$+" ".repeat(Q-$.length)}function l8(){let $=I$();if(!_$($))return process.stdout.write(`${_}No proofs found.${V} Run 'loki start' to generate one.
...
`),process.stderr.write(`Use 'loki proof list' to see available proofs.
`),1;if(X)return a8(Q,q,e(I$(),Q,"proof.json"));if((await R(["gh","--version"],{timeoutMs:5000})).exitCode!==0)return process.stderr.write(`${T}gh CLI not found${V}
...
`),1;if((await R(["gh","auth","status"],{timeoutMs:1e4})).exitCode!==0)return process.stderr.write(`${T}GitHub CLI not authenticated${V}
...
`);let B=`Loki Mode proof-of-run ${Q}`,G=["gh","gist","create",Y,"--desc",B];if(z!=="")G.push(z);let O=await R(G,{timeoutMs:60000});try{g8(H,{recursive:!0,force:!0})}catch{}if(O.exitCode!==0)return process.stderr.write(`${T}Failed to create gist${V}
`),process.stderr.write(`${O.stdout}${O.stderr}
`),1;return process.stdout.write(`${S}Shared: ${O.stdout.trim()}${V}
`),0}async function t8($){let Q=$[0],Z=$.slice(1);if(Q===void 0||Q==="help"||Q==="--help"||Q==="-h")return process.stdout.write(p8),Q===void 0?1:0;switch(Q){case"list":return l8();case"show":return d8(Z[0]);case"open":return o8(Z[0]);case"share":return s8(Z);default:return process.stderr.write(`${T}Unknown subcommand: ${Q}${V}
`),process.stderr.write(`Run 'loki proof --help' for usage.
`),1}}var p8;var T6=P(()=>{C();d();c();G6();p8=`${k}loki proof${V} - inspect and share proof-of-run artifacts
...
`});var L6={};b(L6,{runCrash:()=>K7});import{existsSync as A6,readdirSync as r8,readFileSync as i8}from"fs";import{join as w6}from"path";function L$($){return $===void 0||$===null?"-":String($)}function f$($,Q){return $.length>=Q?$:$+" ".repeat(Q-$.length)}function _6(){return w6(j(),"crash")}function I1(){let $=_6();if(!A6($))return[];try{return r8($,{withFileTypes:!0}).filter((Q)=>Q.isFile()&&Q.name.endsWith(".json")).map((Q)=>Q.name.slice(0,-5)).sort()}catch{return[]}}function $7($){if($.length===0)return!1;if($.includes("/")||$.includes("\\"))return!1;if($.includes(".."))return!1;return!0}function u$($){if(!$7($))return null;let Q=w6(_6(),`${$}.json`);if(!A6(Q))return null;try{return JSON.parse(i8(Q,"utf8"))}catch{return{}}}function Q7(){let $=I1();if($.length===0)return process.stdout.write(`${_}No crash reports found.${V} Nothing has been captured in .loki/crash/.
...
`),0}function I6($){let Q=u$($);if(Q!==null)return{id:$,report:Q};for(let Z of I1()){let z=u$(Z);if(z&&String(z.fingerprint??"")===$)return{id:Z,report:z}}return null}function Z7($){if(!$)return process.stderr.write(`${T}Missing crash id.${V} Use 'loki crash' to list reports.
...
`),0}function z7($){let Q=L$($.error_class),Z=L$($.fingerprint),z=Z!=="-"?Z.slice(0,12):"unknown",X=`crash: ${Q} (${z})`,K=["Anonymous crash report captured by Loki Mode (scrubbed, whitelist-only).","","Scrubbed payload:","```json",JSON.stringify($,null,2),"```","","Nothing was sent automatically. This issue is submitted manually by me."].join(`
...
`),0;let z=Z[Z.length-1],X=u$(z)??{};Q={id:z,report:X}}return process.stdout.write(`${k}Scrubbed payload (this is the ENTIRE report):${V}
...
`),process.stdout.write(O6),2}}var e8="https://github.com/asklokesh/loki-mode/issues/new",O6;var P6=P(()=>{C();c();O6=`${k}loki crash${V} - inspect and manually submit local crash reports
...
`});var k6={};b(k6,{runWiki:()=>U7});import{existsSync as L1,readFileSync as j6}from"fs";import{join as P1,resolve as q7}from"path";function J7(){return P1(process.cwd(),".loki","wiki")}function W7($){let Q="";for(let X of $){if(X==="--help"||X==="-h")return process.stdout.write(`Usage: loki wiki show [section]
...
`),1;return process.stdout.write(j6(z,"utf8")),0}async function R6($,Q){let Z=q7(g,"autonomy","loki"),z=3600000,X=Bun.spawn({cmd:[Z,"wiki",$,...Q],stdin:"inherit",stdout:"inherit",stderr:"inherit",env:{...process.env,LOKI_LEGACY_BASH:"1"}}),q=setTimeout(()=>{try{X.kill("SIGKILL")}catch{}},3600000);try{return await X.exited}finally{clearTimeout(q)}}async function U7($){let Q=$[0],Z=$.slice(1);switch(Q){case void 0:case"help":case"--help":case"-h":return process.stdout.write(F6),0;case"show":return W7(Z);case"generate":return R6("generate",Z);case"ask":return R6("ask",Z);default:return process.stderr.write(`${T}Unknown wiki command: ${Q}${V}
`),process.stdout.write(F6),1}}var F6,V7;var x6=P(()=>{C();c();F6=`${k}loki wiki${V} - Auto-generated, cited codebase wiki + Q&A
...
`),2;let Z=j();try{let X=(await Promise.resolve().then(() => (p$(),F1))).loadPreviousFindings(Z,Q);if(X.findings.length===0)return process.stdout.write(`reflect: no findings for iter ${Q} (nothing to do)
`),0;let q=j$(Z,"state");a7(q,{recursive:!0}),A$(j$(q,`findings-${Q}.json`),{review_id:X.reviewId,iteration:Q,findings:X.findings});let K=await Promise.resolve().then(() => (l$(),g6)),W=0,J=0;if(process.env.LOKI_AUTO_LEARNINGS!=="0"){let H=X.findings.filter((Y)=>Y.severity==="Critical"||Y.severity==="High");for(let Y of H)try{await K.appendFromGateFailure(Z,Q,Y,{episodeBridge:null}),W+=1}catch(B){J+=1,process.stderr.write(`reflect: learning append failed for finding ${Y.reviewer}/${Y.severity}: ${B.message}
...
`),0;let K=(await Promise.resolve().then(() => (p$(),F1))).loadPreviousFindings(Z,Q),W=K.findings.filter((M)=>M.severity==="Critical"||M.severity==="High");if(W.length===0)return process.stdout.write(`override: no blocking findings for iter ${Q} (skip)
`),0;let J=new Set(["duplicate-code-path","file-exists","test-passes","grep-miss","out-of-scope"]),U=async(M)=>{let L=J.has(M.evidence.proofType);return{judge:M.judge,verdict:L?"APPROVE_OVERRIDE":"REJECT_OVERRIDE",reasoning:L?`[stub] proofType=${M.evidence.proofType} trusted`:`[stub] proofType=${M.evidence.proofType} requires manual review`}},H=await z.runOverrideCouncil(W,X,U);await z.recordOverrideOutcome(Z,Q,H,W);let Y=j$(Z,"quality","reviews");if(n7(Y))try{let M=s7(Y).filter((x)=>x.startsWith("review-")).sort(),L=M[M.length-1];if(L&&t7(j$(Y,L)).isDirectory())A$(j$(Y,L,`override-${Q}.json`),{review_id:K.reviewId,iteration:Q,approved_finding_ids:Array.from(H.approvedFindingIds),rejected_finding_ids:Array.from(H.rejectedFindingIds),votes:H.votes})}catch{}let B=H.approvedFindingIds.size,G=H.rejectedFindingIds.size;if(G===0&&B>0)process.stdout.write(`override: LIFTED -- ${B} approved, ${G} rejected
...
`,ZZ;var a6=P(()=>{C();m$();ZZ=x1});t$();function C1(){return process.stdout.write(`Loki Mode v${R$()}
`),0}d();c();C();import{readFileSync as UQ,existsSync as HQ}from"fs";import{resolve as BQ}from"path";var GQ=["claude","codex","cline","aider"];function h1(){let $=BQ(j(),"state","provider");if(!HQ($))return"";try{return UQ($,"utf-8").trim()}catch{return""}}function YQ($,Q){return $||Q||process.env.LOKI_PROVIDER||"claude"}function MQ($){let Q=h1(),Z=YQ($,Q);switch(process.stdout.write(`${k}Current Provider${V}
`),process.stdout.write(`
...
`),0}async function TQ(){let Q=h1()||process.env.LOKI_PROVIDER||"claude";process.stdout.write(`${k}Available Providers${V}
`),process.stdout.write(`
...
`),0}function OQ(){return process.stdout.write(`${k}Loki Mode Provider Management${V}
`),process.stdout.write(`
...
`),0;return process.stdout.write(Z.stdout),0}async function m1($){switch($[0]??"list"){case"list":case"ls":return LQ();case"index":return PQ($[1]==="rebuild");default:{let Z=J$(g,"autonomy","loki"),z=3600000,X=Bun.spawn({cmd:[Z,"memory",...$],stdin:"inherit",stdout:"inherit",stderr:"inherit",env:{...process.env,LOKI_LEGACY_BASH:"1"}}),q=setTimeout(()=>{try{X.kill("SIGKILL")}catch{}},3600000);try{return await X.exited}finally{clearTimeout(q)}}}}C();V$();d();import{resolve as jQ,join as FQ}from"path";import{existsSync as e$,readFileSync as RQ}from"fs";import{homedir as kQ}from"os";import{spawnSync as c1}from"child_process";var p1=3000;function xQ(){let $=(process.env.LOKI_TELEMETRY??"").toLowerCase();if($==="off")return!1;if(process.env.LOKI_TELEMETRY_DISABLED==="true")return!1;if(process.env.DO_NOT_TRACK==="1")return!1;let Q=!1,Z=!1;try{let z=FQ(kQ(),".loki","config");if(e$(z)){let X=RQ(z,"utf8");for(let q of X.split(`
`)){let K=q.replace(/\r$/,"");if(K==="TELEMETRY_DISABLED=true")Q=!0;if(K==="TELEMETRY_ENABLED=true")Z=!0}}}catch{}if(Q)return!1;if($==="on"||Z)return!0;return!1}var k$=!1;function EQ(){return jQ(g,"autonomy","lib","crash_capture.py")}function NQ($,Q){let Z=[$,"--error-class",Q.errorClass,"--message",Q.message];if(Q.stack!==void 0)Z.push("--stack",Q.stack);if(Q.rarvPhase!==void 0)Z.push("--rarv-phase",Q.rarvPhase);if(Q.exitCode!==void 0)Z.push("--exit-code",String(Q.exitCode));if(Q.frictionKind!==void 0)Z.push("--friction-kind",Q.frictionKind);return Z.push("--target-dir",Q.targetDir??process.cwd()),Z}function SQ(){if(e$("/opt/homebrew/bin/python3.12"))return"/opt/homebrew/bin/python3.12";for(let Q of["python3.12","python3"])try{let Z=c1("sh",["-c",`command -v ${Q}`],{timeout:p1,encoding:"utf8"});if(Z.status===0){let z=(Z.stdout||"").trim();if(z)return z}}catch{}return null}function g1($){try{if(!xQ())return;let Q=EQ();if(!e$(Q))return;let Z=SQ();if(!Z)return;let z=NQ(Q,$);c1(Z,z,{timeout:p1,stdio:"ignore"})}catch{}}function f1($,Q){if($ instanceof Error){let z={errorClass:$.name&&$.name.length>0?$.name:Q,message:$.message};if($.stack)z.stack=$.stack;return z}return{errorClass:Q,message:String($)}}var u1=!1;function l1(){if(u1)return;u1=!0,process.on("uncaughtException",($)=>{if(!k$){k$=!0;let Q=f1($,"UncaughtException");g1({errorClass:Q.errorClass,message:Q.message,...Q.stack!==void 0?{stack:Q.stack}:{},exitCode:1})}try{process.stderr.write(`${$&&$.stack||String($)}
...
//# debugId=A8BC9D918F42840F64756E2164756E21
