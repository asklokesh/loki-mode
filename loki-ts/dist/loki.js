...
`)}let J=D($,"queue","pending.json");if(y(J)){let B=await s1('if type == "array" then length elif .tasks then .tasks | length else 0 end',J);process.stdout.write(`${I}Pending Tasks:${V} ${B??"0"}
`)}let U=D($,"metrics","budget.json");if(y(U)){let B=r1(U,"budget_limit"),O=r1(U,"budget_used");if(B!=="0"){process.stdout.write(`${I}Budget:${V} $${O} / $${B}
`);let M=Math.trunc((Number.parseFloat(O)||0)*100),L=Number.parseFloat(B),x=Number.isFinite(L)&&L!==0?Math.trunc(L*100):100,v=a1(M,x,"Budget");if(v!==null)process.stdout.write(`${v}
`)}else process.stdout.write(`${I}Cost:${V} $${O} (no limit)
`)}let H=D($,"state","context-usage.json");if(y(H)){let B=t1(H,"window_size",200000),O=t1(H,"used_tokens",0),M=a1(O,B,"Context");if(M!==null)process.stdout.write(`${M}
`)}let G=[D($,"dashboard","dashboard.pid"),D(bQ(),".loki","dashboard","dashboard.pid")].find((B)=>y(B))??"";if(G&&y(G)){let B=N$(G);if(B!==null&&E$(B)){let O=D(G,".."),M=(u,w)=>{let E=D(O,u);try{return y(E)?W$(E,"utf-8").trim()||w:w}catch{return w}},L=M("scheme","http"),x=M("host","127.0.0.1"),v=M("port",process.env.LOKI_DASHBOARD_PORT||"57374");if(x==="0.0.0.0")x="127.0.0.1";process.stdout.write(`${I}Dashboard:${V} ${L}://${x}:${v}/
...
`),D$($,O.status);let M=J1(),L=M.available_gb===null?null:Math.floor(M.available_gb);if(L===null)process.stdout.write(`  ${A("warn")}  Disk space: unable to determine
...
`),2;return await W6(z,Z.includes("--force"))}case"latest":{let z=w1(),X=z[z.length-1];if(!X)return process.stderr.write(`${T}No checkpoints found to roll back to.${V}
...
`);let U=I1(Q);if(U){let M=p($$(U.cost).usd),L=p($$(U.files_changed).count),x=p($$(U.council).final_verdict),v=$$(U.redaction);process.stdout.write(`  - cost.usd:        ${M}
...
`);let G=`Loki Mode proof-of-run ${Q}`,B=["gh","gist","create",Y,"--desc",G];if(z!=="")B.push(z);let O=await R(B,{timeoutMs:60000});try{f8(H,{recursive:!0,force:!0})}catch{}if(O.exitCode!==0)return process.stderr.write(`${T}Failed to create gist${V}
...
`),1}}var l8;var T6=P(()=>{C();d();c();B6();l8=`${k}loki proof${V} - inspect and share proof-of-run artifacts
...
`,zZ;var s6=P(()=>{C();m$();n6={fn:null};zZ=x1});t$();function C1(){return process.stdout.write(`Loki Mode v${R$()}
`),0}d();c();C();import{readFileSync as HQ,existsSync as GQ}from"fs";import{resolve as BQ}from"path";var YQ=["claude","codex","cline","aider"];function h1(){let $=BQ(j(),"state","provider");if(!GQ($))return"";try{return HQ($,"utf-8").trim()}catch{return""}}function MQ($,Q){return $||Q||process.env.LOKI_PROVIDER||"claude"}function TQ($){let Q=h1(),Z=MQ($,Q);switch(process.stdout.write(`${k}Current Provider${V}
`),process.stdout.write(`
...
`))if(z.includes('"description"'))Z++;return Z}catch{return 0}}async function PQ(){await LQ(M$,{recursive:!0});let $=e$(J$(M$,"patterns.jsonl")),Q=e$(J$(M$,"mistakes.jsonl")),Z=e$(J$(M$,"successes.jsonl"));return process.stdout.write(`${k}Cross-Project Learnings${V}
`),process.stdout.write(`
...
//# debugId=3B1D90644F1C826064756E2164756E21
